/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-12-16 16:17:07                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 540                                        memory_used (MB): 51.1                            
...
void upd(int node,int lo,int hi,int lt,int rt,int vl)
{
  ...
  {
    seg[node].cnt+=vl;
    if(seg[node].cnt)seg[node].sm=hi-lo+1;
    else seg[node].sm=seg[node].dsm;
    return ;
  }
  int md=(lo+hi)/2;
