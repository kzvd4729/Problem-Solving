/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-01-20 22:46:26                      
...
using namespace std;
const long long N=1000,inf=1e12;
...
int main()
{
  ...
  while(t--)
  {
    ...
    for(long long i=1;i<=n;i++)
    {
      ...
      {
        if(bb[j]>=aa[i])
          Hun[i][j]=min(g,(abs((bb[j]-aa[i])-k)+1)*(abs((bb[j]-aa[i])-k))+1);
        else Hun[i][j]=inf;
      }
    }
    cout<<"Case "<<++tc<<": ";
    ...
    for(long long i=1;i<=n;i++)
    {
      ...
      {
        if(bb[j]>=aa[i])
          Hun[i][j]=C-min(g,(abs((bb[j]-aa[i])-k)+1)*(abs((bb[j]-aa[i])-k))+1);
        else Hun[i][j]=inf;
