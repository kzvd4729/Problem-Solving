/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-03-02 01:11:55                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 122                                        memory_used (MB): 7.1                             
...
int main()
{
  ...
  while(t--)
  {
   ...
   p.convexHull();cout<<"Case "<<++tc<<": ";
   if(p.h.size()<3)cout<<0<<endl;
   else
   {
    double ans=1e9;
    //p.h.push_back(p.h[0]);
    ...
    {
     point p1=p.h[(i-1+n)%n]-p.h[i];
     point p2=p.h[(i+1+n)%n]-p.h[i];
     double a=modifiedatan2(p1.x,p1.y);
     double b=modifiedatan2(p2.x,p2.y);
     //cout<<a<<" "<<b<<endl;
     ans=min(ans,fabs(a-b));
    }
