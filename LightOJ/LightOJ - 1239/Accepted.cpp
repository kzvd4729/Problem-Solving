/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-03-02 17:59:32                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 434                                        memory_used (MB): 6.2                             
...
struct polygon
{
  ...
  */
  ...
  double perimeter()
  {
    convexHull();h.push_back(h[0]);int n=h.size();
    double d=0;
    for(int i=0;i<n-1;i++)d+=h[i].dist(h[i+1]);
    return d;
  }
};
int main()
{
  ...
  while(t--)
  {
    int n;double d;cin>>n>>d;polygon pl;pl.p.resize(n);
    for(int i=0;i<n;i++)cin>>pl.p[i].x>>pl.p[i].y;

    cout<<"Case "<<++tc<<": "<<setprecision(8)<<fixed<<pl.perimeter()+2*pi*d<<endl;
  }
