/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-05-09 17:25:21                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 2058                                       memory_used (MB): 6.4                             
  ...
  while(t--)
  {
    ...
    for(int k=c.size()-1;k>=0;k--)
    {
      ...
      for(int i=a.size();i>=0;i--)
      {
        ...
        {
          if(a[i]==c[k])qm[j]=(qm[j]+dp[i+1][j][k+1])%mod;
          if(b[j]==c[k])sm=(sm+dp[i][j+1][k+1])%mod;
          ...
          now=(now+qm[j])%mod;now=(now+sm)%mod;
          dp[i][j][k]=now;
        }
      }
    }
