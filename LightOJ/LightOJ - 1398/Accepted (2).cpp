/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-01-21 17:38:29                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 27                                         memory_used (MB): 3                               
*  problem: https://vjudge.net/problem/LightOJ-1398
****************************************************************************************/
#include<bits/stdc++.h>
...
int main()
{
  ...
  while(t--)
  {
    int l,w,n,d;cin>>l>>w>>n>>d;sr=0,sn=2*n+1;
    for(int i=1;i<=n;i++)cin>>xx[i]>>yy[i];

    for(int i=1;i<=n;i++)
    {
      addEdge(2*i-1,2*i,1);
      if(yy[i]+d>=w)addEdge(sr,2*i-1,1);
      if(yy[i]<=d)addEdge(2*i,sn,1);
      for(int j=1;j<=n;j++)
      {
        if((i!=j)&&(ds(i,j)<=4*d*d))
          addEdge(2*i,2*j-1,1);
