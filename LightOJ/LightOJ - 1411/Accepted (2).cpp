/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-06-24 19:04:01                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 1370                                       memory_used (MB): 50.9                            
...
void upd(int node,int lo,int hi,int lt,int rt,int ty,int vl)
{
  ...
  if(lo>=lt&&hi<=rt)
  {
    ...
    {
      lzd[node]=vl;seg[node]=0;
      lza[node]=0;lzb[node]=0;lzc[node]=0;
    }
    too_lazy(node,lo,hi);
    return ;
  }
  int md=(lo+hi)/2;
  upd(node*2,lo,md,lt,rt,ty,vl);
  upd(node*2+1,md+1,hi,lt,rt,ty,vl);
...
int main()
{
  ...
  while(t--)
  {
    ...
    while(q--)
    {
      ...
      {
        int lt,rt,vl;scanf("%d%d%d",&lt,&rt,&vl);
        upd(1,1,N,lt,rt,3,vl);
      }
      else
      {
        int lt,rt;scanf("%d%d",&lt,&rt);
