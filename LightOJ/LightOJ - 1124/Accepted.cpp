/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-02-17 00:56:58                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 200                                        memory_used (MB): 17.7                            
...
int main()
{
  ...
  while(t--)
  {
    ...
    for(int i=0;i<n;i++)
      scanf("%d%d",&lo[i],&hi[i]);
    int sum=0;
    for(int i=0;i<n;i++)sum+=lo[i];
    if(sum>m)
    {
      printf("Case %d: %d\n",++tc,0);
      continue;
    }
    sum=0;for(int i=0;i<n;i++)sum+=hi[i];
    if(sum<m)
    {
      printf("Case %d: %d\n",++tc,0);
      continue;
    }
    sum=m;for(int i=0;i<n;i++)sum-=lo[i];
    ...
    for(int i=1;i<(1<<n);i++)
    {
      ...
      for(int j=0;j<n;j++)
      {
        if((i&(1<<j)))sum-=(hi[j]+1),cnt++;
        else sum-=lo[j];
      }
      if(sum>=0)
      {
        if(cnt%2==0)tt=(tt+ncr(sum+n-1,n-1))%mod;
