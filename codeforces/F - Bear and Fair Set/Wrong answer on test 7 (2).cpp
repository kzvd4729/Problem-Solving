/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/09/2020 20:57                        
*  solution_verdict: Wrong answer on test 7                  language: GNU C++14                               
*  run_time: 46 ms                                           memory_used: 19700 KB                             
...
int fr[5+2],q;
void add(int l,int r,int cnt,int nm)
{
  if(cnt>r-l+1)no();
...
int main()
{
  ...
  {
    pair<int,int>x=v[i];x.second-=tt;
    addEdge(sr,i+1,x.second);tt+=x.second;
    add(ls+1,x.first,x.second,i+1);
    ls=x.first;
  }
  if(tt>n)no();
  addEdge(sr,q+1,n-tt);
