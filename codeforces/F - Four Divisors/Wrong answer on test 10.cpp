/****************************************************************************************
*  @author: kzvd4729                                         created: Jan/16/2019 22:35                        
*  solution_verdict: Wrong answer on test 10                 language: GNU C++14                               
*  run_time: 156 ms                                          memory_used: 50900 KB                             
...
const int MAXN=1e6,PHI_N=1e5,PHI_K=1e2;
int Pcnt,Primes[MAXN+2],Counter[MAXN+2];
int IsPrime[MAXN+2];
void Seive(int n)
{
  IsPrime[1]=1;
...
int phidp[PHI_N][PHI_K];
void LehmerInit(void)
{
  Seive(MAXN);int k,n,res;
  ...
  int main()
{
  ios_base::sync_with_stdio(0);cin.tie(0);
   long n;cin>>n;LehmerInit();long ans=0;
  for(int i=0; ;i++)
  {
    long x=Primes[i];
