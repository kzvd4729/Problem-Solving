/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/01/2020 13:45                        
*  solution_verdict: Time limit exceeded on test 15          language: GNU C++14                               
*  run_time: 4000 ms                                         memory_used: 9100 KB                              
...
int main()
{
  ...
  double ans=ck(point(0,-100));
  for(int i=1;i<=n;i++)
  {
    double ang=270;point v(0,-100);
    for(int j=1;j<=n;j++)
    {
      if(a[j].y>=a[i].y)continue;
      if(a[j].x1>a[i].x1||a[j].x2<a[i].x1)continue;
      point tmp=point(a[j].x2,a[j].y)-point(a[i].x1,a[i].y);
      if(modifiedatan2(tmp)>270)
        v=tmp,ang=modifiedatan2(tmp);
    }
    ans=min(ans,ck(v));
  }
  for(int i=1;i<=n;i++)
  {
    ...
    {
      if(a[j].y>=a[i].y)continue;
      if(a[j].x1>a[i].x2||a[j].x2<a[i].x2)continue;
      point tmp=point(a[j].x1,a[j].y)-point(a[i].x2,a[i].y);
      if(modifiedatan2(tmp)<270)
        v=tmp,ang=modifiedatan2(tmp);
    }
    ans=min(ans,ck(v));
  ...
  for(int i=2;i<=n;i++)
  {
    if(a[i].y==a[1].y)continue;
    ans=min(ans,ck(point(a[i].x1,a[i].y)-point(a[1].x1,a[1].y)));
    ans=min(ans,ck(point(a[i].x2,a[i].y)-point(a[1].x1,a[1].y)));
    ans=min(ans,ck(point(a[i].x1,a[i].y)-point(a[1].x2,a[1].y)));
    ans=min(ans,ck(point(a[i].x2,a[i].y)-point(a[1].x2,a[1].y)));
  }
  for(int i=1;i<=n;i++)
  {
    for(int j=1;j<=n;j++)
    {
      if(a[j].y>=a[i].y)continue;
      ans=min(ans,ck(point(a[j].x2,a[j].y)-point(a[i].x1,a[i].y)));
      ans=min(ans,ck(point(a[j].x1,a[j].y)-point(a[i].x2,a[i].y)));
    }
  }
  //assert(ans<1e10);
