/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/01/2020 14:00                        
*  solution_verdict: Time limit exceeded on test 15          language: GNU C++14                               
*  run_time: 4000 ms                                         memory_used: 30600 KB                             
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    for(int j=1;j<=n;j++)
    {
      ...
      v.push_back({a[j].x2-a[i].x1,a[j].y-a[i].y});
      v.push_back({a[j].x1-a[i].x2,a[j].y-a[i].y});
    }
  }
  v.push_back({0,-1});
  for(auto &x:v)
  ...
  setp(10);
  for(auto x:v)ans=min(ans,ck(point(x.first,x.second)));
  cout<<ans<<endl;
  return 0;
}