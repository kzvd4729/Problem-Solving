/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/18/2018 20:16                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 1528 ms                                         memory_used: 188200 KB                            
...
using namespace std;
const int N=2e3;
int n,h,aa[N+2],vis[2][N+1][N+1][2];
double p,dp[2][N+1][N+1][2];
double dfs(int s1,int lo,int hi,int s2)
{
  if(vis[s1][lo][hi][s2])return dp[s1][lo][hi][s2];
  vis[s1][lo][hi][s2]=1;
   if(lo>hi)return 0;
   double here=0;
   ///left left
  int em;
  if(s1==0)em=min(aa[lo]-aa[lo-1],h);
  else em=min(aa[lo]-aa[lo-1]-h,h);
  here+=0.5*p*em+0.5*p*dfs(0,lo+1,hi,s2);
   ///left right
  em=aa[lo]+h;int i;
  for(i=lo+1;i<=hi;i++)
  {
  ...
  if(i<lo)
  {
    if(s1==0)em=max(em,aa[lo-1]);
    else em=max(em,aa[lo-1]+h);
  }
  em=aa[hi]-em;
  here+=0.5*p*em+0.5*p*dfs(s1,lo,i,0);
   return dp[s1][lo][hi][s2]=here;
 }
int main()
{
  ...
  cout<<setprecision(9)<<fixed<<dfs(0,1,n,1)<<endl;
  return 0;
}