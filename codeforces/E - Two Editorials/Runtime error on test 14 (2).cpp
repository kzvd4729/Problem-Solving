/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/25/2020 22:29                        
*  solution_verdict: Runtime error on test 14                language: GNU C++14                               
*  run_time: 156 ms                                          memory_used: 300 KB                               
...
const long N=2e3+2;
long sm[N+2];
void add1(long l,long r,long vl)
{
  sm[l]+=vl;sm[r+1]-=vl;
}
vector<long>bf[N+2];long die[N+2];
void add2(long l,long vl)
{
  bf[l].push_back(vl);
  die[l+vl]++;
}
vector<long>fb[N+2];long dei[N+2];
void add3(long r,long vl)
{
  fb[r].push_back(vl);
  dei[r-vl]++;
}
long cal(long n)
{
  for(long i=1;i<=n;i++)sm[i]+=sm[i-1];
  long s=0,a=0;
  for(long i=1;i<=n;i++)
  {
...
int main()
{
  ...
  for(long i=1;i<=n;i++)
  {
    ...
    for(long j=1;j<=m;j++)
    {
      long x=max(i,l[j]),y=min(i+k-1,r[j]);
      long fr=max(0LL,y-x+1);now+=fr;
