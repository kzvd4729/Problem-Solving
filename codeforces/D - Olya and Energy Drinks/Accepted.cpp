/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/07/2017 10:53                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 62 ms                                           memory_used: 33600 KB                             
...
int main()
{
     ...
     while(q.size())
    {
        ...
        {
            if(j+b>m)break;
            if(mat[a][j+b]=='#'||dis[a][j+b]<=dis[a][b])break;
            dis[a][j+b]=min(dis[a][j+b],dis[a][b]+1);
            if(vis[a][j+b]==0)q.push({a,j+b});
            vis[a][j+b]=1;
        }
        for(long j=1;j<=k;j++)
        {
        ...
        {
            if(b-j<1)break;
            if(mat[a][b-j]=='#'||dis[a][b-j]<=dis[a][b])break;
            dis[a][b-j]=min(dis[a][b-j],dis[a][b]+1);
            if(vis[a][b-j]==0)q.push({a,b-j});
            vis[a][b-j]=1;
        }
        for(long j=1;j<=k;j++)
        {
