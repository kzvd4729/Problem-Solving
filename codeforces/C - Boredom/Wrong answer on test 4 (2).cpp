/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/31/2019 20:30                        
*  solution_verdict: Wrong answer on test 4                  language: GNU C++14                               
*  run_time: 1154 ms                                         memory_used: 62800 KB                             
...
int main()
{
  ...
  for(auto x:qr)
  {
    ...
    ans-=1LL*(x[0]-1)*(x[0]-2)/2;
     int p2=get(root[x[2]],root[x[0]-1],1,n,x[3]+1,n);
    int p1=get(root[x[2]],root[x[0]-1],1,n,1,x[1]-1);
     //cout<<p1<<" "<<p2<<endl;
    //cout<<"*"<<ans<<endl;
     ans-=1LL*p1*(p1-1)/2;ans-=1LL*p2*(p2-1)/2;
    ans-=1LL*p1*(x[1]-1-p1);ans-=1LL*p2*(n-x[3]-p2);
     int p3=get(root[x[0]-1],0,1,n,1,x[1]-1);
    int p4=get(root[x[0]-1],0,1,n,x[3]+1,n);
     ans-=1LL*p3*(x[1]-1-(p1+p3));ans-=1LL*p4*(n-x[3]-(p2+p4));
     prnt[x[4]]=ans;
  }
  for(int i=0;i<q;i++)
    cout<<prnt[i]<<" ";
