/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/19/2018 02:32                        
*  solution_verdict: Accepted                                language: GNU C++17                               
*  run_time: 327 ms                                          memory_used: 148100 KB                            
...
long n,fac[N+2],xx,cnt,mp[77],cod[N+2],cev[N+2];
...
long dfs(long st,long msk)
{
  ...
  long here=0;
  here=(cev[arr[st]]*dfs(st+1,msk))%mod;
  here=(here+(cod[arr[st]]*dfs(st+1,msk^fsk[arr[st]]))%mod)%mod;
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    for(int j=0;j<=fr[arr[i]];j++)
    {
      if(j%2==0)cev[arr[i]]=(cev[arr[i]]+ncr(fr[arr[i]],j))%mod;
      else cod[arr[i]]=(cod[arr[i]]+ncr(fr[arr[i]],j))%mod;
    }
  }
