/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/21/2020 15:19                        
*  solution_verdict: Time limit exceeded on test 11          language: GNU C++14                               
*  run_time: 2000 ms                                         memory_used: 241600 KB                            
...
int buf[N+2],ind,buf2[N+2],ind2;
static inline int h(long x){return (((x^179)*17LL)%sz+sz)%sz;}
void init()
{
  for(int i=0;i<2;i++)
...
{
  int k=h(x);
  while(key[id][k]!=inv&&key[id][k]!=x){k++;if(k==sz)k=0;}//first non-taken index
