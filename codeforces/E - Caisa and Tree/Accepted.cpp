/****************************************************************************************
*  @author: kzvd4729                                         created: Feb/07/2020 23:27                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 4976 ms                                         memory_used: 212700 KB                            
...
int aa[N+2],n,vis[N+2],ans[N+2],lv[N+2],prnt[N+2];
...
void dfs(int node,int par,int l)
{
  ...
  if(ans[node]!=-1)prnt[node]=lv[ans[node]];
  for(auto &x:fc[aa[node]])
    st[x].insert(l);
  for(auto x:adj[node])
...
void upd()
{
  for(int i=1;i<=n;i++)ans[i]=-1,prnt[i]=-1,st[i].clear();
  dfs(1,-1,1);
}
int main()
{
  ...
  {
    int ty;cin>>ty;
    if(ty==1){int v;cin>>v;cout<<prnt[v]<<"\n";}
    else {int v,w;cin>>v>>w;aa[v]=w;upd();}
  }
  return 0;
}
void seive()
