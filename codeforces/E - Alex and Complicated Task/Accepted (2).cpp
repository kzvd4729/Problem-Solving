/****************************************************************************************
*  @author: kzvd4729#                                        created: May/29/2020 18:30                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 1310 ms                                         memory_used: 201700 KB                            
...
void build(int node,int lo,int hi)
{
  if(lo==hi)
  {
    seg[node].push_back(rt[lo]);
    if(rt[lo]<=n)seg[node].push_back(rt[rt[lo]]);
    return ;
  }
  int md=(lo+hi)/2;
...
int main()
{
  ...
  for(int i=n;i>=1;i--)
  {
    ...
    int nx=rt[i];if(nx>n)continue;
    nx=rt[nx];if(nx>n)continue;
    nx=rt[nx];if(nx>n)continue;
    dp[i]=max(dp[i],dp[nx+1]+1);
    g[i].push_back(aa[nx]);
    g[i].push_back(aa[nx]);
    g[i].push_back(nx);
  }
  ...
  {
    if(dp[pt]==dp[pt+1])pt++;
    else if(f[pt].size()&&dp[pt]==dp[f[pt][2]+1]+1)
    {
      cout<<rm[f[pt][0]]<<" "<<rm[f[pt][1]]<<" "<<rm[f[pt][0]]<<" "<<rm[f[pt][1]]<<" ";
      cnt++;
      pt=f[pt][2]+1;
    }
    else if(g[pt].size()&&dp[pt]==dp[g[pt][2]+1]+1)
    {
      cout<<rm[g[pt][0]]<<" "<<rm[g[pt][1]]<<" "<<rm[g[pt][0]]<<" "<<rm[g[pt][1]]<<" ";
      cnt++;
      pt=g[pt][2]+1;
    }
    else assert(0);
