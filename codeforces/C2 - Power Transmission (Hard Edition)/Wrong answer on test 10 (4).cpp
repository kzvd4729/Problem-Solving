/****************************************************************************************
*  @author: kzvd4729                                         created: May/10/2019 01:50                        
*  solution_verdict: Wrong answer on test 10                 language: GNU C++14                               
*  run_time: 358 ms                                          memory_used: 26900 KB                             
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    for(int j=i+1;j<=n;j++)
    {
      ...
      ans+=st.size()-mp[{eq.a/__gcd(abs(eq.a),abs(eq.b)),eq.b/__gcd(abs(eq.a),abs(eq.b))}];
      st.insert(p);mp[{eq.a/__gcd(abs(eq.a),abs(eq.b)),eq.b/__gcd(abs(eq.a),abs(eq.b))}]++;
    }
  }
  cout<<ans<<endl;
  return 0;
