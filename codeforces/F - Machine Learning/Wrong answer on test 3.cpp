/****************************************************************************************
*  @author: kzvd4729                                         created: Feb/03/2021 17:07                        
*  solution_verdict: Wrong answer on test 3                  language: GNU C++14                               
*  run_time: 3540 ms                                         memory_used: 39100 KB                             
...
int main()
{
  ...
  for(int i=1;i<=cnt;i++)
  {
     ...
     while(id<qr[i].id)
    {
      ...
      if(p[id])
      {
        if(p[id]>=qr[i].l&&p[id]<=qr[i].r)add(vl[id],1);
        if(p[id]>=qr[i].l&&p[id]<=qr[i].r)add(a[p[id]],-1);
        int tmp=a[p[id]];
        a[p[id]]=vl[id];
         vl[id]=tmp;
      }
