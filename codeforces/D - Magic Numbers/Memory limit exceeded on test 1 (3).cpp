/****************************************************************************************
*  @author: kzvd4729                                         created: Apr/05/2018 17:18                        
*  solution_verdict: Memory limit exceeded on test 1         language: GNU C++14                               
*  run_time: 93 ms                                           memory_used: 262100 KB                            
...
int dfs(int st,int ck,int rl,int md,int last)
{
  ...
  if(dp[st][rl%2][md][last]!=-1)return dp[st][rl%2][md][last];
  long cnt=0;
  if(rl&1)
  {
  ...
  return dp[st][rl%2][md][last]=cnt%mod;
}
