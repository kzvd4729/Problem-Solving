/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/19/2020 14:18                        
*  solution_verdict: Time limit exceeded on test 11          language: GNU C++17                               
*  run_time: 1000 ms                                         memory_used: 76000 KB                             
...
int main()
{
  ...
  vector<int>g;
  for(int i=1;i<=n;i++)if(adj[i].size()>=k)g.push_back(i);
  for(auto x:g)tryCatch(x);
  vector<int>v;
