/****************************************************************************************
*  @author: kzvd4729                                         created: Feb/15/2019 22:20                        
*  solution_verdict: Time limit exceeded on test 8           language: GNU C++14                               
*  run_time: 3000 ms                                         memory_used: 24300 KB                             
...
unordered_map<pair<int,int>,long,HASH>mp[N+2];
long dfs(int,int,int);
long dfs2(int rt,int ms,int msk)
{
  if(mp[rt].count({ms,msk}))return mp[rt][{ms,msk}];
