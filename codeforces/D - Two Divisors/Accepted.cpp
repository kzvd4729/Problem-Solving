/****************************************************************************************
*  @author: kzvd4729                                         created: Jun/11/2020 23:50                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 530 ms                                          memory_used: 139000 KB                            
...
void fac(int n)
{
  ...
  //for(int i=1;i<=id;i++)cout<<f[i]<<" ";cout<<endl;
  //memset(vs,0,sizeof vs);
}
int a[N+2],lt[R+2],rt[R+2],tmp[N+2];
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    ...
    if(ff==0)lt[x]=-1,rt[x]=-1;
    if(id>=2&&lt[x]==-1)
    {
      //for(int j=1;j<=id;j++)tmp[j]=1;
      //for(int j=1;j<=id;j++)
      //{
        //while(x%f[j]==0)tmp[j]*=f[j],x/=f[j];
      //}
      for(int j=1;j<=id;j++)
      {
        for(int k=1;k<=id;k++)
        {
          int ad=f[j]+x/f[k];int g=0;//cout<<ad<<endl;
          for(int l=1;l<=id;l++)
            if(ad%f[l]==0){g=1;break;}
          if(g==0){lt[x]=f[j],rt[x]=x/f[k];ff=1;break;}
        }
        if(ff)break;
      }
      if(ff)continue;
      ...
      for(int j=1;j<=id;j++)
      {
        for(int k=1;k<=id;k++)
        {
          int ad=tmp[j]+x/tmp[k];int g=0;//cout<<ad<<endl;
          for(int l=1;l<=id;l++)
            if(ad%f[l]==0){g=1;break;}
          if(g==0){lt[x]=tmp[j],rt[x]=x/tmp[k];ff=1;break;}
        }
        if(ff)break;
      }
      if(ff==0)cout<<a[i]<<endl;
    }
  }
