/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/01/2020 14:56                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 140 ms                                          memory_used: 105200 KB                            
...
int main()
{
    ...
    for(int j=1;j<=m;j++)
    {
      for(int t=1;t<=m/2;t++)
      {
        for(int md=0;md<k;md++)
        {
          ...
          d[i][j][t][md]=max(d[i][j][t][md],d[i][j-1][t-1][((md-a[i][j])%k+k)%k]+a[i][j]);
        }
      }
    }
  }
  ...
  for(int i=1;i<=n;i++)
  {
    for(int j=0;j<k;j++)
    {
      for(int l=0;l<k;l++)
      {
        dp[i][j]=max(dp[i][j],dp[i-1][((j-l)%k+k)%k]+d[i][m][m/2][l]);
      }
    }
  }
  cout<<dp[n][0]<<endl;
