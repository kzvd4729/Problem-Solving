/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/14/2019 20:12                        
*  solution_verdict: Wrong answer on test 8                  language: GNU C++14                               
*  run_time: 46 ms                                           memory_used: 64300 KB                             
...
int dfs(int cl,int rw,int a,int b,int c)
{
  ...
  for(int j=rw;j<l;j++)
  {
    ...
    {
      if(ta&(1<<j))ta^=(1<<j);
      now=min(now,cost[0]+dfs(cl,j+1,ta,tb,tc));
    }
    else if(j-rw==1)
    {
      if(ta&(1<<j))ta^=(1<<j);
      ...
      now=min(now,cost[1]+dfs(cl,j+1,ta,tb,tc));
    }
    else
    {
      if(ta&(1<<j))ta^=(1<<j);
      ...
      now=min(now,cost[2]+dfs(cl,j+1,ta,tb,tc));
    }
  }
