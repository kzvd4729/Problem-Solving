/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/02/2019 00:52                        
*  solution_verdict: Wrong answer on test 1                  language: GNU C++14                               
*  run_time: 15 ms                                           memory_used: 68900 KB                             
  ...
  for(int i=1;i<=q;i++)
  {
     ...
     if(oc[a].size()<=500&&oc[b].size()<=500)
    {
      ...
      for(int j=0;j<oc[a].size();j++)
      {
        ...
        int mn1=1e9,mn2=1e9;
        if(p+1<oc[b].size())mn1=oc[b][p+1]-min(oc[a][j]-ok[a],oc[b][p+1]-ok[b]);
        mn2=oc[a][j]-min(oc[a][j]-ok[a],oc[b][p]-ok[b]);
