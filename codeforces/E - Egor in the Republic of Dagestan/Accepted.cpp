/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/08/2020 23:26                        
*  solution_verdict: Accepted                                language: GNU C++17                               
*  run_time: 529 ms                                          memory_used: 42000 KB                             
...
vector<pair<int,int> >ad[N+2],rv[N+2],pr[N+2];
    ...
    for(auto x:rv[u])
    {
      if(x.second==0)
      {
        if(max(a[u],b[u])+1<a[x.first])
        {
          ...
          {
            pr[x.first].push_back({u,x.second});
            q.push(x.first);
          }
        }
      }
...
int main()
{
  ...
  {
    int u,v,w;cin>>u>>v>>w;
    rv[v].push_back({u,w});ad[u].push_back({v,w});
  }
  solve();
   return 0;
}