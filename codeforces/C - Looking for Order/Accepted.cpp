/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/17/2018 00:22                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 77 ms                                           memory_used: 65700 KB                             
...
int dfs(int st,int msk)
{
  ...
  for(int j=0;j<n;j++)
  {
    ...
    for(nx=st+1;nx<n;nx++)
    {
      if(nx==j||msk&(1<<nx))continue;
      break;
    }
    here=min(here,mat[st][j]+dfs(nx,(msk|(1<<st))|(1<<j)));
  }
  return dp[msk]=here;
}
int main()
