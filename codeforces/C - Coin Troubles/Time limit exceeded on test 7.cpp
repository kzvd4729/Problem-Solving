/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/22/2018 16:01                        
...
int dfs(int st,int vl,int f)
{
    ...
    if(dp[f][st][vl]!=-1)return dp[f][st][vl];
    int here=0;
    if(f==0)
    {
        here=(here+dfs(st,vl-mm[lin[st]][1],1))%mod;
        if(parent[lin[st]]==parent[lin[st+1]])
        {
            here+=dfs(st+1,vl-mm[lin[st+1]][1],1);
        }
        else here+=dfs(st+1,vl,0);
    }
    else
    {
        here+=dfs(st,vl-mm[lin[st]][0],1);
        if(parent[lin[st]]==parent[lin[st+1]])
        {
            here+=dfs(st+1,vl,1);
        }
