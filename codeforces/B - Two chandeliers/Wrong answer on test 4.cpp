/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/13/2021 18:35                        
...
int main()
{
  ...
  while(hi-lo>2)
  {
    ...
    for(int i=0;i<gc;i++)
    {
      ...
      while(true)
      {
        if(pt+1<(int)dq.size() && dq[pt+1]<=md)
        {
          pt++;
          fr[a[(dq[pt]-1)%n+1]]++;
      ...
      {
        now+=fr[b[j+1]];
          int id=dq.back();dq.pop_back();
        fr[a[(id-1)%n+1]]++;
        dq.push_front(id);if(pt<(int)dq.size()-1)pt++;
         sub+=gc;
        if(dq[pt]+sub>md)
        {
          fr[a[(dq[pt]-1)%n+1]]--;
