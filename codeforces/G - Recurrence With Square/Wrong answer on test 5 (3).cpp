...
*  run_time: 30 ms                                           memory_used: 4000 KB                              
...
{
  long d=a.size(),e=a[0].size();
  long f=b.size(),g=b[0].size();
...
{
  long n=a.size();if(a[0].size()!=n)assert(0);//impossible
  v2d ret;ret.resize(n);
  for(long i=0;i<n;i++)ret[i].resize(n);
  for(long i=0;i<n;i++)ret[i][i]=1;
  while(p)
  {
...
int main()
{
  ...
  long n;long k;cin>>n>>k;
   for(long i=0;i<n;i++)cin>>a[i];
  for(long i=0;i<n;i++)cin>>c[i];
   long p,q,r;cin>>p>>q>>r;
   if(k<n)cout<<a[k]<<endl,exit(0);
    v2d b(n+6);
  for(long i=0;i<n+6;i++)b[i].resize(n+6);
   long qq=(1LL*n*q)%mod;
  long rr=(1LL*n*n*r)%mod;
  long ir=(1LL*n*r)%mod;
   for(long i=0;i<n;i++)b[0][i]=c[0];
  for(long i=n;i<n+3;i++)b[0][i]=1;
   for(long i=1;i<n;i++)b[i][i-1]=1;
   b[n][n]=1;
  ...
  b[n+5][n+5]=1;b[n+5][n+4]=1;
   b=expo(b,k-n+1);
   long ans=0;
  for(long i=0;i<n;i++)ans=(ans+1LL*b[0][i]*a[n-i-1])%mod;
