/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/28/2020 19:54                        
...
int main()
{
  ...
  for(int i=0;i<n;i++)cin>>c[i];
   ...
   if(k<n)cout<<a[k]<<endl,exit(0);
    v2d b(n+7);
  for(int i=0;i<n+7;i++)b[i].resize(n+7);
   int qq=(1LL*n*q)%mod;
  ...
  b[n+2][n+2]=1;b[n+2][n+6]=2;b[n+2][n+4]=1;
  b[n+3][n+3]=1;
  b[n+4][n+4]=1;
  b[n+5][n+5]=1;
  b[n+6][n+6]=1;b[n+6][n+4]=1;
   b=expo(b,k-n+1);
   int ans=0;
  ...
  ans=(ans+1LL*b[0][n+5])%mod;
  ans=(ans+1LL*ir*b[0][n+6])%mod;
   cout<<ans<<endl;
     return 0;
}