/****************************************************************************************
*  @author: kzvd4729                                         created: May/07/2020 15:43                        
*  solution_verdict: Wrong answer on test 10                 language: GNU C++14                               
*  run_time: 233 ms                                          memory_used: 27200 KB                             
...
void dds(int node,int par,int pr)
{
  ...
  {
    if(x==par)continue;
    int con=(1LL*dp[node]*big(dp[x]+1,mod-2))%mod;
...
int main()
{
  ...
  for(int i=1;i<=n;i++)cout<<(ans[i]+mod)%mod<<" ";cout<<endl;
  return 0;
}