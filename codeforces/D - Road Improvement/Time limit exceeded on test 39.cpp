/****************************************************************************************
*  @author: kzvd4729                                         created: May/07/2020 16:18                        
*  solution_verdict: Time limit exceeded on test 39          language: GNU C++14                               
*  run_time: 2000 ms                                         memory_used: 41000 KB                             
...
void dds(int node,int par,int pr)
{
  ...
  for(int i=1;i<adj[node].size();i++)
  {
    ...
    for(int j=1;j<adj[node].size();j++)
      if(i!=j)con=(1LL*con*(dp[adj[node][j]]+1))%mod;
