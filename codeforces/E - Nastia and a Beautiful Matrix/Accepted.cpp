/****************************************************************************************
*  @author: kzvd4729                                         created: May/08/2021 00:11                        
*  solution_verdict: Accepted                                language: GNU C++17 (64)                          
*  run_time: 421 ms                                          memory_used: 25000 KB                             
 ...
 bool ck(int n)
{
  ...
  for(int i=1;i<=n;i+=2)
  {
    for(int j=2;j<=n;j+=2)
    {
      ...
      if(pt<=k)
      {
        v[i][j]=tmp[pt].second,tmp[pt].first--;
        //cout<<tmp[pt].first<<endl;
      }
    }
  }
   //for(int i=1;i<=k;i++)cout<<tmp[i].first<<" ";cout<<endl;
  pt=1;
  for(int i=2;i<=n;i+=2)
  {
    for(int j=1;j<=n;j+=2)
