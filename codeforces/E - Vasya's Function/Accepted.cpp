/****************************************************************************************
*  @author: kzvd4729                                         created: May/28/2020 17:59                        
...
int main()
{
  ...
  if(a==1)cout<<b<<endl,exit(0);
   long ans=0;int cur;for(int i=0;i<v.size();i++)if(__gcd(a,b)==v[i])cur=i;
  while(b)
  {
    long mx=0,id=v.size();
    for(int i=cur+1;i<v.size();i++)
    {
      __int128 x=v[cur],y=v[i];
      __int128 lcm=(x*y)/(__gcd(x,y));
