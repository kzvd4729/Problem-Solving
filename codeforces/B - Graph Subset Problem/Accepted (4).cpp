/****************************************************************************************
*  @author: kzvd4729                                         created: Dec/03/2020 21:35                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 577 ms                                          memory_used: 21700 KB                             
...
int d[N+2];
vector<int>ad[N+2];unordered_set<int>da[N+2];
int main()
{
  ...
  while(t--)
  {
    ...
    while((int)st.size())
    {
      ...
      if(p.first==k-1)
      {
        ...
        if(f==1){tr.push_back(p.second);ans=tr;break;}
      }
       st.erase(p);
