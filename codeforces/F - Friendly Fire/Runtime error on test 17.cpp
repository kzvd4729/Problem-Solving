/****************************************************************************************
*  @author: kzvd4729                                         created: Dec/10/2019 19:57                        
*  solution_verdict: Runtime error on test 17                language: GNU C++14                               
...
int main()
{
  ...
  for(int i=0;i<n;i++)
  {
    ...
    v[i][2]=i+1;
     mp[bb[i+1]].push_back({aa[i+1],i+1});
  }
   sort(v.begin(),v.end());
   // for(auto x:v)
   ...
   for(auto x:mp)
  {
    ...
    if(x.second.size()>=2)
    {
      ...
      int now=0;
      if(x.second[sz-1].first>=x.first&&x.second[sz-2].first>=x.first)
        now=x.second[sz-1].first+x.second[sz-2].first;
      if(now>ans)ans=now,a=x.second[sz-1].second;
    }
  }
  for(int i=0;i<n-1;i++)
  {
