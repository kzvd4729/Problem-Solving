/****************************************************************************************
*  @author: kzvd4729                                         created: Apr/05/2019 20:43                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 3494 ms                                         memory_used: 231500 KB                            
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    for(int j=1;j<=n;j++)
    {
      ...
      for(int k=0;k<min(10,(int)ba[i].size());k++)
      {
        if(ba[i][k].second==i||ba[i][k].second==j)continue;
        for(int l=0;l<min(10,(int)ab[j].size());l++)
        {
