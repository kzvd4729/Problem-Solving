/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/30/2019 13:23                        
*  solution_verdict: Wrong answer on test 10                 language: GNU C++14                               
*  run_time: 15 ms                                           memory_used: 53200 KB                             
*  problem: https://codeforces.com/gym/102001/problem/H
****************************************************************************************/
#include<bits/stdc++.h>
#define long long long
using namespace std;
const long N=4e5,inf=1e9;
long aa[N+2],qm[N+2],zr[N+2];
...
void upd(long node,long lo,long hi,long id,long vl)
{
 ...
 long lt[N+2],rt[N+2],con[N+2],sm[N+2];
void no(){cout<<"Impossible"<<endl,exit(0);}
int main()
{
  ios_base::sync_with_stdio(0);cin.tie(0);
  long n,m;cin>>n>>m;
  for(long i=1;i<=n;i++)cin>>aa[i],qm[i]=qm[i-1]+aa[i],zr[i]=zr[i-1]+(aa[i]==0);
   vector<pair<long,long> >v;
  for(long i=1;i<=m;i++)
  {
    long x,y;cin>>x>>y>>sm[i];lt[i]=x,rt[i]=y;
    v.push_back({x,-i});v.push_back({y,i});
  }
  sort(v.begin(),v.end());
  for(long i=0;i<5*N+2;i++)seg[i]=inf;
   long cnt=0,pt=0;
  for(long i=1;i<=n;i++)
  {
    while(pt<v.size())
    {
      ...
      {
        long id=-v[pt].second;
        ++cnt;long mx=qm[rt[id]]-qm[lt[id]-1]+zr[rt[id]]-zr[lt[id]-1];
