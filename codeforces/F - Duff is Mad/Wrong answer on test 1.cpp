/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/28/2019 20:23                        
*  solution_verdict: Wrong answer on test 1                  language: GNU C++14                               
*  run_time: 31 ms                                           memory_used: 16500 KB                             
...
struct ahoCorasick
{
  ...
  {
    trie.resize(n+1);
    tree.resize(n+1);cnt.resize(n+1);
    for(int i=0;i<=n;i++)
    {
  ...
  void abc(string &s)
  {
    ...
    {
      now=trie[now].next[x-'a'];
      for(link=now;link;link=trie[link].link)
      {
        occur[link]++;
        clr.push_back(link);
      }
