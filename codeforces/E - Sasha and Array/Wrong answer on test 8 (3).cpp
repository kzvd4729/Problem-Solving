/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/27/2018 13:18                        
*  solution_verdict: Wrong answer on test 8                  language: GNU C++17                               
*  run_time: 31 ms                                           memory_used: 8600 KB                              
...
int n,m,arr[100005],nw,nx,lazy[400005],ty,l,r,x;
struct segment
{
  int now,next;
}seg[400005];
...
matrix multiply(matrix a,matrix b)
{
  ...
  for(int i=1;i<=2;i++)
  {
    for(int j=1;j<=2;j++)
    {
      ...
      for(int k=1;k<=2;k++)
      {
        c.mat[i][j]=(c.mat[i][j]*1LL+(a.mat[i][k]*1LL)*(b.mat[k][j]*1LL))%mod;
      }
    }
  }
  return c;
...
void upd(int node,int lo,int hi,int lt,int rt,int in)
{
  if(lazy[node])
  {
    ...
    if(lo!=hi)
    {
      lazy[node*2]=(lazy[node*2]*1LL+lazy[node]*1LL)%mod;
      lazy[node*2+1]=(lazy[node*2+1]*1LL+lazy[node]*1LL)%mod;
    }
    lazy[node]=0;
  }
  ...
  if(lo>=lt&&hi<=rt)
  {
    ...
    if(lo!=hi)
    {
      lazy[node*2]=(lazy[node*2]*1LL+in*1LL)%mod;
      lazy[node*2+1]=(lazy[node*2+1]*1LL+in*1LL)%mod;
    }
    return ;
  }
  int md=(lo+hi)/2;
