/****************************************************************************************
*  @author: kzvd4729                                         created: Feb/23/2018 20:56                        
*  solution_verdict: Memory limit exceeded on test 8         language: GNU C++14                               
*  run_time: 482 ms                                          memory_used: 262100 KB                            
...
const long N=300000;
long fb[N+2],n,m,arr[N+2],seg[4*N],ty,l,r,ll,rr,md;
vector<pair<long,long> >lazy[4*N];
void fibonacci(void)
{
    fb[1]=1;
    fb[2]=1;
...
void upd(long node,long lo,long hi,long l,long r)
{
    ...
    {
        ll=lazy[node][lazy[node].size()-1].first;
        rr=lazy[node][lazy[node].size()-1].second;
        lazy[node].pop_back();
        seg[node]=(seg[node]+fb[rr]+mod-fb[ll-1])%mod;
        if(lo!=hi)
        {
            md=(ll+rr)/2;
            lazy[node*2].push_back({ll,md});
            lazy[node*2+1].push_back({md+1,rr});
        }
    }
    if(lo>r||hi<l)return ;
    if(lo>=l&&hi<=r)
