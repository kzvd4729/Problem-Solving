/****************************************************************************************
*  @author: kzvd4729                                         created: Feb/23/2018 21:00                        
*  solution_verdict: Memory limit exceeded on test 1         language: GNU C++14                               
*  run_time: 249 ms                                          memory_used: 262100 KB                            
...
void upd(long node,long lo,long hi,long l,long r)
{
    ...
    {
        ll=lazy[node].front().first;
        rr=lazy[node].front().second;
        lazy[node].pop();
        seg[node]=(seg[node]+fb[rr]+mod-fb[ll-1])%mod;
        if(lo!=hi)
        {
            md=(ll+rr)/2;
            lazy[node*2].push({ll,md});
            lazy[node*2+1].push({md+1,rr});
        }
    }
    if(lo>r||hi<l)return ;
    if(lo>=l&&hi<=r)
