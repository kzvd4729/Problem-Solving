/****************************************************************************************
*  @author: kzvd4729                                         created: Feb/23/2018 23:11                        
*  solution_verdict: Memory limit exceeded on test 8         language: GNU C++14                               
*  run_time: 1450 ms                                         memory_used: 262100 KB                            
...
void propagate(long node,long lo,long hi)
{
    if(lo==hi)return ;
    while(lazy[node].size())
    {
    ...
    long mid=(lo+hi)/2;
    propagate(node*2,lo,mid);
    propagate(node*2+1,mid+1,hi);
}
int main()
{
