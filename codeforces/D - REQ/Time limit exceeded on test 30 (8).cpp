/****************************************************************************************
*  @author: kzvd4729                                         created: Apr/03/2019 18:59                        
*  solution_verdict: Time limit exceeded on test 30          language: GNU C++17                               
*  run_time: 3000 ms                                         memory_used: 81700 KB                             
*  problem: https://codeforces.com/contest/594/problem/D
****************************************************************************************/
#include<bits/stdc++.h>
#define long long long
using namespace std;
const int N=1e6,sq=450;
const long mod=1e9+7;
int vis[N+2],aa[N+2],fr[N+2],mrk[N+2];
...
int main()
{
  ...
  factorise();int n;scanf("%d",&n);
  for(int i=1;i<=n;i++)scanf("%d",&aa[i]);
  int q;scanf("%d",&q);
  for(int i=1;i<=q;i++)
  {
    scanf("%d%d",&qr[i].fr,&qr[i].sc);
    qr[i].id=i;
  }
  ...
  {
    nn=(nn*aa[i])%mod;mrk[aa[i]]++;
    if(mrk[aa[i]]>1)continue;
    for(auto x:v[aa[i]])
    {
  ...
  for(int ii=2;ii<=q;ii++)
  {
    ...
    {
      nn=(nn*inv[aa[i]])%mod;mrk[aa[i]]--;
      if(mrk[aa[i]]>0)continue;
      for(auto x:v[aa[i]])
      {
