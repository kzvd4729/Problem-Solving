/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/23/2019 16:18                        
*  solution_verdict: Wrong answer on test 21                 language: GNU C++14                               
*  run_time: 1060 ms                                         memory_used: 16000 KB                             
...
using namespace std;
const int N=1e6;
const int mod=1004535809,r=3;//generator//primitive root
int big(int b,int p)
{
  int ret=1;
  while(p)
 ...
 int main()
{
  ...
  int n,k;cin>>n>>k;vector<int>a(N+1,0);
  for(int i=1;i<=n;i++)
  {
    int x;cin>>x;a[x]=1;
  }
