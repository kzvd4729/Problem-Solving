/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/23/2020 16:51                        
*  solution_verdict: Wrong answer on test 3                  language: GNU C++14                               
*  run_time: 61 ms                                           memory_used: 236400 KB                            
...
void add(int c,int k,int f,int tm)
{
  ...
  {
    if(ls[i]==-1)ls[i]=tm;
    else if(fr[i]>0)ans[fr[i]]+=min(k,tm-ls[i]);
    fr[i]+=f;ls[i]=tm;
  }
  ans[0]=0;
}
int main()
{
  ...
  for(int i=1;i<=n;i++)add(pp[i].second,k,-1,pp[i].first+k);
