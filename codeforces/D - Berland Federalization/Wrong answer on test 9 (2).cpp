/****************************************************************************************
*  @author: kzvd4729                                         created: May/01/2019 00:23                        
*  solution_verdict: Wrong answer on test 9                  language: GNU C++14                               
*  run_time: 30 ms                                           memory_used: 1400 KB                              
...
int n,k,dp[N+2][N+2],tmp[N+2][N+2],parent[N+2],mn;
...
void print(int node,int k)
{
  ...
  for(int l=sz-1;l>=1;l--)
  {
    ...
    {
      if(ed.size()<mn)ed.push_back({node,x});
      continue;
    }
    for(int i=1;i<k;i++)
    {
  ...
  if(k==1&&ed.size()<mn)ed.push_back({node,nex[node][0]});
