/****************************************************************************************
*  @author: kzvd4729                                         created: Jan/28/2019 20:42                        
...
long dfs(int i,int j,int ex)
{
  ...
  if(i>j)return aa[ex];if(dp[i][j][ex]!=-1)return dp[i][j][ex];
  long now=0;
  if(ex)now=max(now,aa[ex]+dfs(i,j,0));
  for(int k=i;k<j;k++)
    now=max(now,dfs(i,k,ex)+dfs(k+1,j,0));
  now=max(now,aa[v[i]+ex]+dfs(i+1,j,0));
  now=max(now,aa[v[j]]+dfs(i,j-1,ex));
  for(int k=i+2;k<=j;k+=2)
  {
    now=max(now,aa[ex+v[i]+v[k]]+dfs(i+1,k-1,0)+dfs(k+1,j,0));
    now=max(now,dfs(i+1,k-1,0)+dfs(k,j,ex+v[i]));
    if(k+1<=j)
