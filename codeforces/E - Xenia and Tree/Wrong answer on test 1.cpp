/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/13/2018 21:38                        
*  solution_verdict: Wrong answer on test 1                  language: GNU C++14                               
*  run_time: 0 ms                                            memory_used: 3100 KB                              
...
int main()
{
  ...
  while(m--)
  {
    ...
    if(ty==1)
    {
      ...
      while(nd!=-1)
      {
        ans[nd]=min(ans[nd],ds+=abs(lev[nd]-lev[tmp]));
        nd=anc[nd];
      }
    }
    else
    {
      int tmp=nd,pr=inf,ds=0;
      while(nd!=-1)
      {
        pr=min(pr,ds+=abs(lev[nd]-lev[tmp])+ans[nd]);
        nd=anc[nd];
      }
      cout<<pr<<endl;
    }
