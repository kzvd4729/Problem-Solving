/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/26/2020 19:04                        
*  solution_verdict: Wrong answer on test 11                 language: GNU C++14                               
*  run_time: 248 ms                                          memory_used: 216000 KB                            
...
{
  if(i==n&&j==n-1&&k==n)return 0;
  if(dp[i][j][k][f]!=mod)return dp[i][j][k][f];
  int ret=-mod;
  if(i<n)ret=max(ret,a[i+1][j]+a[i+1][k]+dfs(i+1,j,k,0));
  if(f==0&&j<k-1)ret=max(ret,a[i][j+1]+dfs(i,j+1,k,0));
...
int main()
{
  ...
  cout<<a[1][1]+a[1][2]+dfs(1,1,2,0)<<endl;
  return 0;
}