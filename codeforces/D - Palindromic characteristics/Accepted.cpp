/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/22/2020 14:32                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 1091 ms                                         memory_used: 197300 KB                            
...
int z[5000+2][5000+2];
void zFunction(int id,string &s)
{
  int n=s.size();
  z[id][id]=0;//l,r maintains a window with maximum r
  for(int i=1,l=0,r=0;i<n;i++)
  {
    if(i<=r)z[id][id+i]=min(r-i+1,z[id][id+i-l]);//inside the window
    while(i+z[id][id+i]<n&&s[z[id][id+i]]==s[i+z[id][id+i]])z[id][id+i]++;//brute-force
    if(i+z[id][id+i]-1>r)l=i,r=i+z[id][id+i]-1;//r update
  }
}
int main()
{
  ...
  for(int k=1;k<=16;k++)
  {
    ...
    for(int i=0;i<n;i++)
    {
      for(int j=i+1;j<n;j++)
      {
        ...
        if(z[i][j-md+1]>=md)dp[i][j][k]=1,cnt++;
