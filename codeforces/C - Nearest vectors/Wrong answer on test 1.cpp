/****************************************************************************************
*  @author: kzvd4729                                         created: Jan/18/2019 20:33                        
...
{
  //have problem above 90 degree
  //Dont trust this function
  //double ck=Angle_Between_Two_Vector_Unsigned_Atan2(a,b);
  //if(fabs(ck-180.0)<=0.000000000001)return DegreetoRadian(ck);
...
int main()
{
  ios_base::sync_with_stdio(0);cin.tie(0);
   /*point f={-1,0};
  point g={-0,-1};
  cout<<Angle_Between_Two_Vector_Unsigned_Atan2(f,g)<<endl;
  cout<<Angle_Between_Two_Vector_Unsigned_Cos(f,g)<<endl;*/
    int n;cin>>n;
  for(int i=1;i<=n;i++)
  ...
  for(int i=2;i<=n;i++)
  {
    ...
    if(an<90)an=Angle_Between_Two_Vector_Unsigned_Cos(c,d);
    if(an<mx)
    {
      mx=an;
