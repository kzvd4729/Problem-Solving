/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/29/2020 21:09                        
*  solution_verdict: Wrong answer on test 7                  language: GNU C++14                               
*  run_time: 982 ms                                          memory_used: 62200 KB                             
...
int add(int u,int v)
{
  ...
  if(p1==p2)return 0;
   ...
   if(sz[p1]<sz[p2])
  {
    par[p1]=p2;sz[p2]+=sz[p1];//sz[p1]=0;
    return u;
  }
  else 
  {
    par[p2]=p1;sz[p1]+=sz[p2];//sz[p2]=0;
    return v;
  }
}
  ...
  for(int i=1;i<=q;i++)
  {
    ...
    else
    {
      ...
      ad[u[nm]].erase(v[nm]);ad[v[nm]].erase(u[nm]);
       //cout<<xx[i]<<" "<<u[nm]<<" "<<v[nm]<<endl;
       //cout<<par[u[nm]]<<" ** "<<par[v[nm]]<<endl;
      if(xx[i]==u[nm])
      {
        if(u[nm]!=par[u[nm]])dds(u[nm],-1,u[nm],par[u[nm]]);
        else dds(v[nm],-1,v[nm],par[v[nm]]);
      }
      else
      {
        if(v[nm]!=par[v[nm]])dds(v[nm],-1,v[nm],par[v[nm]]);
        else dds(u[nm],-1,u[nm],par[u[nm]]);
      }
    }
    //cout<<i<<" ";
    //for(int i=1;i<=n;i++)cout<<par[i]<<" ";cout<<endl;
