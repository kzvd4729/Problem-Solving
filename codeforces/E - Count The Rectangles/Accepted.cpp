/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/15/2019 09:54                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 1387 ms                                         memory_used: 2800 KB                              
...
int Find(int node,int lo,int hi,int pnode,int vl)
{
  if(lo>=vl)return Seg[node].sm-Seg[pnode].sm;
  if(hi<vl)return 0;
  int md=(lo+hi)/2;
  return Find(Seg[node].l,lo,md,Seg[pnode].l,vl)+Find(Seg[node].r,md+1,hi,Seg[pnode].r,vl);
}
 int ver,hor;
struct vertical
{
...
int main()
{
  ...
  for(int i=1;i<=ver;i++)
  {
    ...
    for(int j=1;j<=ver;j++)
    {
      ...
      int now=Find(Rt[v[rt]],1,N,Rt[v[lt]],vv[j].c);
      ans+=((now*(now-1))/2);
    }
