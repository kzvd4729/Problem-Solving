/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/23/2018 19:51                        
...
using namespace std;
int bt[100005],cp[100005],n,k,cpp[100005],vis[100005],sum[100005];
long ans;
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    if(vis[i])ans+=(bt[i]*sum[i+1])*1LL;
    else
    {
      ans+=(cpp[i+1]*bt[i])*1LL;
      if(!vis[i+1])ans+=(bt[i]*bt[i+1])*1LL;
      if(i==1)if(!vis[n])ans+=(bt[1]*bt[n])*1LL;
    }
  }
  cout<<ans<<endl;
  return 0;
