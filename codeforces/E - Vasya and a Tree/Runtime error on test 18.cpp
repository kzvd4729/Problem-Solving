/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/13/2018 00:46                        
*  solution_verdict: Runtime error on test 18                language: GNU C++14                               
*  run_time: 732 ms                                          memory_used: 68400 KB                             
...
void dds(int node,int par,int h)
{
  ...
  upd(ent[node],hgt[h]);upd(ext[node]+1,-hgt[h]);
  for(auto x:adj[node])
  {
    if(x==par)continue;
    dds(x,node,h+1);
