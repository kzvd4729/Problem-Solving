/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/25/2019 12:14                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 2620 ms                                         memory_used: 63300 KB                             
...
int main()
{
 ...
 for(int i=1;i<=n;i++)aa[i]=cn[aa[i]];
  for(int i=1;i<=n;i++)fr[aa[i]]++,pos[aa[i]].push_back(i);
 for(int i=n-1;i>=1;i--)fr[i]+=fr[i+1];
  for(int i=1;i<=n;i++)
 {
 ...
 while(q--)
 {
  ...
  int lw=md;
   int i=upper_bound(v.begin(),v.end(),rv[lw])-v.begin();
  int nd=k;
  if(i<v.size())nd-=fr[cn[v[i]]];
   //cout<<nd<<endl;
   lo=1,hi=n,md;
  while(hi-lo>2)
  {
   md=(lo+hi)/2;int tt=get(rot[md],1,n,lw+1);
   tt+=min(nd,upper_bound(pos[lw].begin(),pos[lw].end(),md)-pos[lw].begin());
    if(tt<id)lo=md;
   else hi=md;
  }
  for(md=lo;md<=hi;md++)
  {
   int tt=get(rot[md],1,n,lw+1);
   tt+=min(nd,upper_bound(pos[lw].begin(),pos[lw].end(),md)-pos[lw].begin());
   if(tt>=id)break;
  }
  cout<<rv[aa[md]]<<"\n";
