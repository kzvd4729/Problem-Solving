/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/01/2020 15:31                        
*  solution_verdict: Accepted                                language: GNU C++17                               
*  run_time: 171 ms                                          memory_used: 17100 KB                             
*  problem: https://codeforces.com/edu/course/2/lesson/2/5/practice/contest/269656/problem/D
****************************************************************************************/
#include<iostream>
#include<vector>
#include<cstring>
...
int main()
{
  ...
  long cr=0;stack<pair<int,int> >st;
   ...
   for(int i=1;i<=n;i++)
  {
    ...
    while(st.size())
    {
      if(st.top().first>=a[i])
      {
        cr-=1LL*st.top().first*st.top().second;
        cnt+=st.top().second;st.pop();
      }
      else break;
    }
    cr+=1LL*a[i]*cnt;st.push({a[i],cnt});
    ans+=cr;
  }
  cout<<ans<<endl;
  return 0;
