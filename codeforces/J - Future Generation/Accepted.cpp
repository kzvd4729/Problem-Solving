/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/29/2019 19:38                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 935 ms                                          memory_used: 4500 KB                              
...
bool cmp(int a,int b)
{
  ...
  for(int i=0;i<15;i++)if(b&(1<<i))v.push_back(p[i]);
   for(int i=0;i<min((int)u.size(),(int)v.size());i++)
  {
    if(u[i]<v[i])return true;if(u[i]>v[i])return false;
  }
...
bool cmp2(pair<int,int>a,pair<int,int>b)
{
  ...
  for(int i=0;i<15;i++)if(b.second&(1<<i))v.push_back(s[b.first][i]);
   //cout<<a.second<<" "<<u<<" "<<b.second<<" "<<v<<endl;
   for(int i=0;i<min((int)u.size(),(int)v.size());i++)
  {
    if(u[i]<v[i])return true;if(u[i]>v[i])return false;
  }
