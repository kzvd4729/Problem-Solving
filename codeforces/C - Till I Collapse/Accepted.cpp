/****************************************************************************************
*  @author: kzvd4729                                         created: Feb/15/2018 19:11                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 530 ms                                          memory_used: 240600 KB                            
*  problem: https://codeforces.com/contest/786/problem/C
****************************************************************************************/
#include<bits/stdc++.h>
#define long long long
using namespace std;
long n,arr[100005],last[100005],root[100005],sz,tmp,x,now,ans;
struct segment
{
    long l,r,sum;
} seg[10000000];
...
void query(long node,long lo,long hi,long kk)
{
    ...
    long mid=(lo+hi)/2;
    long x=seg[seg[node].l].sum;
    if(x>=kk)query(seg[node].l,lo,mid,kk);
    else
    {
        kk-=x;
        query(seg[node].r,mid+1,hi,kk);
    }
}
int main()
{
     ...
     for(long i=n; i>=1; i--)
    {
     ...
     ///for(long i=1;i<=n;i++)cout<<seg[root[i]].sum<<endl;
    vector<long>print;
    long pr=0;
    for(long i=1;i<=n;i++)
    {
        ...
        while(true)
        {
            ...
            pr++;
            if(seg[root[now]].sum<=i)break;
            query(root[now],1,n,i+1);
            now=ans;
        }
        print.push_back(pr);
    }
