/****************************************************************************************
*  @author: kzvd4729                                         created: Nov/07/2020 22:41                        
*  solution_verdict: Wrong answer on test 11                 language: GNU C++14                               
*  run_time: 3432 ms                                         memory_used: 3900 KB                              
...
int main()
{
  ...
  else
  {
    for(int i=1;i<=min(n,k);i++)
    {
      ...
      for(int j=1;j<=i;j++)
      {
        int ml=(1LL*x*ncr(k,i))%mod;
        ml=(1LL*ml*x)%mod;
        ml=(1LL*ml*ncr(k-i,i-j))%mod;
        ml=(1LL*ml*ncr(i,j))%mod;
        ml=(1LL*ml*big(j,n*m-n*2))%mod;
        ans=(ans+ml)%mod;
      }
    }
  }
