/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/03/2019 19:44                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 1326 ms                                         memory_used: 39100 KB                             
...
int main()
{
  ...
  sort(aa+1,aa+n+1,cmp);add({0,0});long ans=0;
  for(int i=1;i<=n;i++)
  {
    long now=qry(aa[i].b)+aa[i].a*aa[i].b-aa[i].c;
    add({-aa[i].a,now});ans=max(ans,now);
  }
  cout<<ans<<endl;
  return 0;
}