/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/21/2019 19:55                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 560 ms                                          memory_used: 2400 KB                              
...
int main()
{
  ...
  {
    string tmp;cin>>tmp;
    if(tmp.size()==1||tmp.size()>s.size())continue;
    ...
    {
      if(a[i]==tmp.size()||(id+a[i]<c.size()&&a[i]+c[id+a[i]]>=tmp.size()))
      {
        ans++;break;
      }
      id++;
