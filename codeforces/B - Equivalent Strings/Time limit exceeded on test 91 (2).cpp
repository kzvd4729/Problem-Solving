/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/04/2020 16:04                        
*  solution_verdict: Time limit exceeded on test 91          language: GNU C++14                               
...
bool dfs(int l1,int r1,int l2,int r2)
{
  ...
  int md1=(l1+r1)/2,md2=(l2+r2)/2;
   if(rand()%2==0)
  {
    if(dfs(l1,md1,l2,md2))
    {
      if(dfs(md1+1,r1,md2+1,r2))return true;
    }
    if(dfs(l1,md1,md2+1,r2))
    {
      if(dfs(md1+1,r1,l2,md2))return true;
    }
  }
  else
  {
