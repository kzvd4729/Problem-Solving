/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/01/2020 21:13                        
*  solution_verdict: Wrong answer on test 13                 language: GNU C++14                               
*  run_time: 108 ms                                          memory_used: 36700 KB                             
...
int main()
{
  ...
  while(rm.size())
  {
    ...
    int b=bf.back();bf.pop_back();
    vector<int>dl;
    for(auto x:rm)
    {
      if(st[b].find(x)==st[b].end())dl.push_back(x);
    }
    for(auto x:dl)rm.erase(x),bf.push_back(x);
  }
  cout<<ans<<endl;
    return 0;
}