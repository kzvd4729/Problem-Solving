/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/19/2021 23:56                        
*  solution_verdict: Accepted                                language: GNU C++17 (64)                          
*  run_time: 639 ms                                          memory_used: 75100 KB                             
...
vector<int>findMin(vector<pair<int,int> >v)
{
  ...
  vector<int>ret;int i=id;
  for(int j=v[i].first;j<=v[i].second;j++)fr[a[j]]=1;
  for(int j=v[i+1].first;j<=v[i+1].second;j++)
  {
    if(fr[a[j]]==0)continue;
    if(fr[a[j]]==1)ret.push_back(j);
    fr[a[j]]++;
  }
  for(int j=v[i].first;j<=v[i].second;j++)fr[a[j]]=0;
   return ret;
...
int solve(int n,int k)
{
  while(true)
  {
    ...
    if((int)x.size()>k)return (int)v.size();
     k-=(int)x.size();
     vector<int>tmp;
