/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/09/2018 14:13                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 139 ms                                          memory_used: 8700 KB                              
...
int main()
{
  ...
  for(int i=0;i<n;i++)
  {
    ...
    tt.push_back(dis(a[i],a[(i+2)%n]));
  }
  int id=0;
  for(int i=0;i<n;i++)
  {
    long p=dis(b[i],b[(i+1)%n]);
    long q=dis(b[i],b[(i+2)%n]);
     int now=kmp[id++];
    while(true)
    {
      if(p==mt[now]&&q==tt[now])
      {
        kmp[id]=now+1;
        break;
      }
    ...
    if(kmp[id]==n)cout<<"YES"<<endl,exit(0);
  }
  for(int i=0;i<n;i++)
  {
    long p=dis(b[i],b[(i+1)%n]);
