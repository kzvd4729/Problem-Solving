/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/02/2018 01:20                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 280 ms                                          memory_used: 13500 KB                             
...
using namespace std;
const int N=2e5;
int n,q,ent[N+2],ext[N+2],tim,lin[N+2],lev[N+2];
int bit1[N+2],bit2[N+2];
vector<int>adj[N+2];
void dfs(int node,int par,int lv)
{
  ent[node]=++tim;lin[tim]=node;lev[node]=lv;
  for(auto x:adj[node])
  {
    if(x==par)continue;
    dfs(x,node,lv+1);
