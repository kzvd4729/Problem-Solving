/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/18/2020 17:31                        
*  solution_verdict: Accepted                                language: GNU C++17 (64)                          
*  run_time: 1762 ms                                         memory_used: 25400 KB                             
...
int main()
{
  ...
  assert(v[0].second==0);
   st.add(v[0].first,0);__int128 ans=(m+v[0].first-1)/v[0].first;
  for(int i=1;i<v.size();i++)
  {
    ...
    for(md=lo;md<=hi;md++)if(st.get(md)>=v[i].second)break;
     __int128 now=st.get(md);
    __int128 ex=now-v[i].second;
     st.add(v[i].first,-v[i].first*md+ex);
  }
  __int128 lo=1,hi=m,md;
  while(hi-lo>2)
  {
    md=(lo+hi)/2;
