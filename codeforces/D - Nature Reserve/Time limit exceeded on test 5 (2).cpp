/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/06/2018 20:37                        
*  solution_verdict: Time limit exceeded on test 5           language: GNU C++14                               
*  run_time: 2000 ms                                         memory_used: 800 KB                               
...
{
  double lo=0,hi=1e14,md;int bin=80;
  while(bin--)
  {
    md=(lo+hi)/2.0;int f=0;
    for(int i=1;i<=n;i++)
    {
      if(dis(x,md,pp[i].first*1.0,pp[i].second*1.0)>md)f=1;
    }
...
int main()
{
  ...
  double lo=0,hi=mx,md1,md2;int ter=40;
  while(ter--)
  {
