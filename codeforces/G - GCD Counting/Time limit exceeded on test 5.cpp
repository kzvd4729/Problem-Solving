/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/14/2018 22:36                        
*  solution_verdict: Time limit exceeded on test 5           language: GNU C++14                               
*  run_time: 4500 ms                                         memory_used: 32200 KB                             
...
vector<int>adj[N+2],dvv[N+2],mcl,ccl,vsl;
long ans[N+2];
void divisors(void)
{
  for(int i=1;i<=N;i++)
...
{
  cnt[gcd]++;if(cnt[gcd]==1)mcl.push_back(gcd);
  for(auto x:adj[node])
  {
    if(x==par||mark[x])continue;
