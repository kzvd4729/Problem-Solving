/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/17/2018 23:11                        
*  solution_verdict: Wrong answer on test 31                 language: GNU C++14                               
*  run_time: 1107 ms                                         memory_used: 221100 KB                            
...
void dfs(int node,int par,int msk,int lv)
{
  ...
  for(auto x:adj[node])
  {
    ...
    dfs(x.first,node,msk^(1<<(x.second-(int)'a')),lv+1);
    sz[node]+=sz[x.first];
  }
  ed[node]=tim;
}
int dsu(int node,int par,int clr)
{
  ...
  for(auto x:adj[node])
  {
    ...
    for(int i=st[x.first];i<=ed[x.first];i++)
    {
      ...
      if(f&&ans[2604]==6)cout<<lev[lin[i]]<<" "<<msk<<" "<<sack[msk]<<" "<<lev[node]<<endl,exit(0);
      for(int j=0;j<23;j++)
      {
