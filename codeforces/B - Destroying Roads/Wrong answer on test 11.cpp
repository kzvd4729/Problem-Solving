/****************************************************************************************
*  @author: kzvd4729                                         created: May/04/2020 19:30                        
...
void dfs(int bg,int node)
{
  ...
  if(ds1[bg]+df+dt1[node]<=l1&&ds2[bg]+df+dt2[node]<=l2)
    ans=min(ans,ds1[bg]+df+dt1[node]+ds2[bg]+dt2[node]);
  for(auto x:adj[node])
...
int main()
{
  ...
  ans=ds1[t1]+ds2[t2];
  for(int i=1;i<=n;i++)dfs(i,i);cout<<m-ans<<endl;
  return 0;
}