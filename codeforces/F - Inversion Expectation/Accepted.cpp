/****************************************************************************************
*  @author: kzvd4729                                         created: Dec/31/2018 11:44                        
...
long inv[N+2],fac[N+2],bit[N+2],qm[N+2],aa[N+2],mq[N+2];
void upd(int x)
{
  for( ;x>0;x-=x&-x)
    bit[x]++;
...
int main()
{
  ...
  {
    if(aa[i]==-1)continue;
    long bg=n-aa[i]-(mq[n]-mq[aa[i]]),sm=aa[i]-1-(mq[aa[i]-1]);
    long bp=qm[i-1],sp=qm[n]-qm[i];
     //cout<<bg<<" "<<sm<<" "<<bp<<" "<<sp<<endl;
