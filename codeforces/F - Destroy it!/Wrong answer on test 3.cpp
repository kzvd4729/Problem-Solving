/****************************************************************************************
*  @author: kzvd4729                                         created: Jun/14/2019 22:22                        
*  solution_verdict: Wrong answer on test 3                  language: GNU C++14                               
*  run_time: 171 ms                                          memory_used: 41100 KB                             
...
using namespace std;
const int N=2e5;
int n,aa[N+2],bb[N+2],cc[N+2];long dp[N+2][12];
long dfs(int id,int m)
{
  if(id>n)return 0;
  if(dp[id][m]!=-1)return dp[id][m];
  vector<int>v;
  for(int i=aa[id-1]+1;i<=aa[id];i++)
