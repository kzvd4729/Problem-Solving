/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/02/2020 01:46                        
*  solution_verdict: Accepted                                language: GNU C++17                               
*  run_time: 2979 ms                                         memory_used: 51800 KB                             
...
int cal(int i,int j)
{
  ...
  {
    md=(lo+hi)/2;
    if(l-md*df<0||md*df>r-l+1)hi=md;
    else if(lcp(l-md*df,l)>=md*df)lo=md;
    else hi=md;
  }
  for(md=hi;md>=lo;md--)
  {
    if(l-md*df<0||md*df>r-l+1)continue;
...
int main()
{
  ...
  //   for(int j=i;j<n;j++)
  //   {
  //     if(cal(i,j)!=cal2(i,j))cout<<i<<" "<<j<<endl;
  //   }
  // }
  //cout<<cal(4,5)<<" "<<cal2(4,5)<<endl;exit(0);
  for(int i=2;i<=n;i++)
  {
