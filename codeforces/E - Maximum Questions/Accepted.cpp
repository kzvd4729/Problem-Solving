/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/14/2019 22:22                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 155 ms                                          memory_used: 32500 KB                             
...
int cnt[N+2],ok[N+2],dp[N+2],st[N+2],an[N+2],m;
int dfs(int i)
{
  ...
  if(ok[i]&&dp[i+m]<=dp[i]+1)
    now=min(now,cnt[i]+dfs(max(i+m,st[dp[i]-1])));
  return an[i]=now;
}
int main()
{
  ios_base::sync_with_stdio(0);cin.tie(0);
  int n;cin>>n;string s;cin>>s>>m;
