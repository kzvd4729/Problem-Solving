/****************************************************************************************
*  @author: kzvd4729                                         created: Dec/03/2019 19:21                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 296 ms                                          memory_used: 43300 KB                             
...
int main()
{
  ...
  for(auto x:adj[center])dfs3(x,center,++cnt);
   //for(int i=1;i<=cnt;i++)if(v[i].size()>k)assert(0);
   //cout<<"*"<<cnt<<endl;
  // for(int i=1;i<=cnt;i++)
  //   cout<<"*"<<v[i].size()<<endl;
   priority_queue<pair<int,int> >pq;
  for(int i=1;i<=cnt;i++)if((int)v[i].size())pq.push({(int)v[i].size(),i});
   while(pq.size()>=2)
  {
    ...
    v[p1].pop_back();v[p2].pop_back();
     if((int)v[p1].size())pq.push({(int)v[p1].size(),p1});
    if((int)v[p2].size())pq.push({(int)v[p2].size(),p2});
  }
  if((int)pq.size())
  {
