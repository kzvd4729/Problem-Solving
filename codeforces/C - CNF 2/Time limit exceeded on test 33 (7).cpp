/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/13/2019 20:38                        
*  solution_verdict: Time limit exceeded on test 33          language: GNU C++14                               
*  run_time: 2000 ms                                         memory_used: 65800 KB                             
...
int main()
{
  ...
  {
    if(pos[i].size()==0||neg[i].size()==0)continue;
    if(!vis[pos[i][0]])adj[pos[i][0]].push_back(i);
    if(!vis[neg[i][0]])adj[neg[i][0]].push_back(i);
  }
  int cnt=0;
