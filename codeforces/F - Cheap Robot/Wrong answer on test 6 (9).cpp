/****************************************************************************************
*  @author: kzvd4729                                         created: Jun/28/2020 16:37                        
*  solution_verdict: Wrong answer on test 6                  language: GNU C++17                               
*  run_time: 217 ms                                          memory_used: 128300 KB                            
...
int main()
{
  ...
  vector<vector<long> >ed,tr;vector<int>r;
  for(int i=1;i<=k;i++)r.push_back(i);shuffle(r.begin(),r.end(),rng);
  for(long p=0;p<20;p++)
  {
    vector<long>a,b;
    for(long j=0;j<k;j++)
    {
      if(j&(1<<p))a.push_back(r[j]);
      else b.push_back(r[j]);
    }
    ...
    for(long i=1;i<=n;i++)
    {
      assert(id[i]!=tid[i]);assert(id[i]>=1&&id[i]<=k);assert(tid[i]>=1&&tid[i]<=k);
      vs[id[i]]=1,vs[tid[i]]=1;
      ed.push_back({d[i]+td[i],id[i],tid[i]});
    }
    for(int i=1;i<=k;i++)if(vs[i]==0)assert(0);
  }
