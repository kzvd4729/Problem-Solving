/****************************************************************************************
*  @author: kzvd4729                                         created: Apr/30/2020 17:10                        
*  solution_verdict: Time limit exceeded on test 64          language: GNU C++14                               
*  run_time: 1500 ms                                         memory_used: 4500 KB                              
...
using namespace std;
const int N=3e2,magic=1e2;
int n,m,p,mt[N+2][N+2],ds[N+2][N+2],tmp[N+2][N+2];
...
void goNext(int tr)
{
  ...
  while((int)q.size())
  {
    ...
    if(isV(u+1,v)&&tmp[u][v]+1<tmp[u+1][v])
      tmp[u+1][v]=tmp[u][v]+1,q.push({u+1,v});
     if(isV(u-1,v)&&tmp[u][v]+1<tmp[u-1][v])
      tmp[u-1][v]=tmp[u][v]+1,q.push({u-1,v});
     if(isV(u,v+1)&&tmp[u][v]+1<tmp[u][v+1])
      tmp[u][v+1]=tmp[u][v]+1,q.push({u,v+1});
     if(isV(u,v-1)&&tmp[u][v]+1<tmp[u][v-1])
      tmp[u][v-1]=tmp[u][v]+1,q.push({u,v-1});
  }
