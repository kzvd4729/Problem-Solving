/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/20/2020 00:46                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 2589 ms                                         memory_used: 122000 KB                            
...
{
  for( ;id>0;id-=id&-id)bit[id].clear();
}
...
void solve(int n)
{
  for(int i=1;i<=n;i++)
  {
    ...
    for(auto x:vv[i])
    {
      ...
      {
        int g=get(ent[x[0]],ent[x[1]],ext[x[1]]);
        fl[x[2]]|=(g<=ext[x[1]]);
      }
    }
    for(auto x:vv[i])
    {
      if(x.size()==2)del(ent[x[0]],ent[x[1]]);
    }
  }
}
int main()
{
  ...
  while(m--)
  {
    ...
    vv[lc].push_back({u,v});//vv[lc].push_back({v,u});
  }
  dfs(1,-1);
  for(int j=1;j<=20;j++)
  {
