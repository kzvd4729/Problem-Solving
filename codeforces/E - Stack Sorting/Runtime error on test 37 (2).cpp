/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/21/2020 21:22                        
*  solution_verdict: Runtime error on test 37                language: GNU C++17                               
*  run_time: 77 ms                                           memory_used: 10600 KB                             
...
int main()
{
   ...
   while(st.size())
  {
    ...
    it--;
     sk.push(*it);st.erase(*it);a[++k]=*it;assert(*it);
    while(sk.size())
    {
      if(sk.top()==tr)sk.pop(),tr++;
      else break;
