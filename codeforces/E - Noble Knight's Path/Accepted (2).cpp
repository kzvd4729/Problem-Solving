/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/25/2018 23:13                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 2558 ms                                         memory_used: 103100 KB                            
...
int anc[N+2],tim,ent[N+2],lin[N+2],nw,sp[N+2][22],atc[N+2];
vector<int>adj[N+2];
struct persistent
{
  int l,r,sm;
...
int how_much(int u,int v,int y,int r)
{
  ...
  while(anc[u]!=anc[v])
  {
    ...
    ret-=qry(root[r],1,n,ent[anc[u]],ent[u]);
    ret+=qry(root[y],1,n,ent[anc[u]],ent[u]);
    u=parent[anc[u]];
  }
  ret+=ent[u]-ent[v]+1;
  ret-=qry(root[r],1,n,ent[v],ent[u]);
  ret+=qry(root[y],1,n,ent[v],ent[u]);
  return ret;
}
...
int main()
{
  ...
  for(int i=1;i<=q;i++)
  {
    ...
    if(ty==1)
    {
      ...
      upd(root[i],1,n,root[i-1],ent[x]);
    }
    else
    {
      root[i]=root[i-1];
      int a,b,k,y;cin>>a>>b>>k>>y;
      cout<<finaly(a,b,k,y,i)<<endl;
    }
  }
  return 0;
}