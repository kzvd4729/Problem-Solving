/****************************************************************************************
*  @author: kzvd4729                                         created: Sep/25/2018 21:00                        
*  solution_verdict: Wrong answer on test 3                  language: GNU C++14                               
*  run_time: 30 ms                                           memory_used: 38700 KB                             
...
int anc[N+2],tim,ent[N+2],lin[N+2],nw,sp[22][N+2],atc[N+2];
vector<int>adj[N+2];
struct persistent
{
  int l,r,sm;
...
{
  int lc=lca(u,v);int h=how_much(u,lc,y,r);
...
int main()
{
  ...
  for(int i=1;i<=q;i++)
  {
    ...
    {
      root[i]=root[i-1];
      int a,b,k,y;cin>>a>>b>>k>>y;if(atc[a]<=y)k++;
