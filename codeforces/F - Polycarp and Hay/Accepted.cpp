/****************************************************************************************
*  @author: kzvd4729                                         created: Apr/25/2020 12:22                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 1340 ms                                         memory_used: 94000 KB                             
...
int cnt;
void dfs(int i,int j,int mn)
{
  if(cnt==0)return ;
  if(i>n||i<1||j>m||j<1||vs[i][j]||mt[i][j]<mn)return ;
  cnt--;vs[i][j]=1;
  dfs(i+1,j,mn);dfs(i-1,j,mn);
  dfs(i,j+1,mn);dfs(i,j-1,mn);
}
int main()
{
