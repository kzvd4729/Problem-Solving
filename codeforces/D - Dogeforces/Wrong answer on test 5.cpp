/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/02/2021 22:57                        
*  solution_verdict: Wrong answer on test 5                  language: GNU C++14                               
*  run_time: 31 ms                                           memory_used: 20700 KB                             
...
const int N=500,inf=1e9,mod=1e9+7;
 int k,a[N+2][N+2],sl[1000000+2],pr[1000000+2];
vector<int>ad[1000000+2];
int main()
{
   ...
   for(int i=1;i<=n;i++)
  {
    ...
    for(int j=(int)v.size()-2;j>=0;j--)
    {
      ...
      {
        ++k;sl[k]=v[j];
        ad[nd].push_back(k);pr[k]=nd;
        nd=k;
      }
    }
    ad[nd].push_back(i);pr[i]=nd;
  }
    for(int i=n+1;i<=k;i++)
  {
