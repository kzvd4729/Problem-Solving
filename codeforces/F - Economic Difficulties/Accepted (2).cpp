/****************************************************************************************
*  @author: kzvd4729                                         created: Dec/03/2019 13:34                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 499 ms                                          memory_used: 78600 KB                             
...
int lca(int f,int a,int b)
{
  ...
  if(f==1)
  {
    while(lva[b]>lva[a])cnt++,b=para[b];
    while(lva[a]>lva[b])a=para[a];
    while(a!=b)cnt++,a=para[a],b=para[b];
  }
  else
  {
    while(lvb[b]>lvb[a])cnt++,b=parb[b];
    while(lvb[a]>lvb[b])a=parb[a];
    while(a!=b)cnt++,a=parb[a],b=parb[b];
  }
  return cnt;
}
