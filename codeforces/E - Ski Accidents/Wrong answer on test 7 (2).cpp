/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/02/2020 16:13                        
*  solution_verdict: Wrong answer on test 7                  language: GNU C++17                               
*  run_time: 483 ms                                          memory_used: 47100 KB                             
...
int main()
{
  ...
  while(t--)
  {
    ...
    set<vector<int> >st;
    for(int i=1;i<=n;i++)st.insert({(in[i].size()>0)&&(out[i].size()>0),in[i].size()+out[i].size(),i});
     while(cnt--)
    {
      ...
      int nd=p[2];ans.push_back(nd);
       for(auto x:out[nd])
      {
        st.erase({(in[x].size()>0)&&(out[x].size()>0),in[x].size()+out[x].size(),x});
        in[x].erase(nd);
        st.insert({(in[x].size()>0)&&(out[x].size()>0),in[x].size()+out[x].size(),x});
      }
      for(auto x:in[nd])
      {
        st.erase({(in[x].size()>0)&&(out[x].size()>0),in[x].size()+out[x].size(),x});
