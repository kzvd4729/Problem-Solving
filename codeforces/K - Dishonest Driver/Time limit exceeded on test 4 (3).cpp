/****************************************************************************************
*  @author: kzvd4729                                         created: Jan/03/2020 23:50                        
*  solution_verdict: Time limit exceeded on test 4           language: GNU C++11                               
*  run_time: 6000 ms                                         memory_used: 11700 KB                             
...
//............................................................
int dp[702][702],rem[700+2][700+2];
inline int dfs(int l,int r)
{
  if(l>r)return 0;if(dp[l][r]!=-1)return dp[l][r];
  if(l==r)return dp[l][r]=1;int now=inf;
...
int main()
{
  ...
  cal(N);int n;cin>>n;cin>>hs.s;hs.makeHash();
   for(int i=0;i<n;i++)
  {
    for(int ln=1;ln<n-i;ln++)
    {
      for(int j=i+ln;j+ln-1<n;j+=ln)
      {
        if(hs.segment(i,i+ln-1)!=hs.segment(j,j+ln-1))break;
        rem[i][ln]++;
      }
    }
  }
