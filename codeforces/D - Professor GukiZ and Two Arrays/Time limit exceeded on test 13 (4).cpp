/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/21/2018 22:21                        
*  solution_verdict: Time limit exceeded on test 13          language: GNU C++14                               
*  run_time: 3000 ms                                         memory_used: 90800 KB                             
...
using namespace std;
const int N=2000;
int arr[N+2],srr[N+2],lo,hi,md1,md2,n,m;
...
int main()
{
  ...
  cin.tie(0);
   cin>>n;
  for(int i=1;i<=n;i++)cin>>arr[i],s1+=arr[i]*1LL;
  cin>>m;
  for(int i=1;i<=m;i++)cin>>srr[i],s2+=srr[i]*1LL;
  a={-1,-1};
  b={-1,-1};
  mn=abs(s1-s2);
   one();
