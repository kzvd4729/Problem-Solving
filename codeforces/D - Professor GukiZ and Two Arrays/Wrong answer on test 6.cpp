/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/21/2018 21:58                        
*  solution_verdict: Wrong answer on test 6                  language: GNU C++14                               
*  run_time: 156 ms                                          memory_used: 21800 KB                             
...
using namespace std;
const int N=2000;
int arr[N+2],srr[N+2],s1,s2,ss1,ss2,lo,hi,md1,md2,n,m,mn;
pair<int,int>a,b;
struct data
{
  int vl,r,c;
...
int main()
{
  ...
  cin.tie(0);
   cin>>n;
  for(int i=1;i<=n;i++)cin>>arr[i],s1+=arr[i];
  cin>>m;
  for(int i=1;i<=m;i++)cin>>srr[i],s2+=srr[i];
  a={-1,-1};
  b={-1,-1};
  mn=abs(s1-s2);
   one();
