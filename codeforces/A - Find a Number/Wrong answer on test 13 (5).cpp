/****************************************************************************************
*  @author: JU_Troubleshooer: tariqiitju, the0dd1out, kzvd4729 created: Oct/20/2018 15:43                        
*  solution_verdict: Wrong answer on test 13                 language: GNU C++14                               
*  run_time: 31 ms                                           memory_used: 24300 KB                             
...
int dp(int r,int s)
{
 if(s==0) return fuck[r];
 int &res=mem[r][s];
 if(~res) return res;
 res=INF;
 for(int d= (r==0?1 :0 ); d<=min(9,s);d++)
  if( dp( (r*10+d)%M,s-d ) + 1 < res)
  {
   res=dp( (r*10+d)%M,s-d ) + 1 ;
   ch[r][s]=d;
  }
 return res;
}
