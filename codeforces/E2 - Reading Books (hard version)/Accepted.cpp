/****************************************************************************************
*  @author: * kzvd4729                                       created: Jun/28/2020 22:27                        
*  solution_verdict: Accepted                                language: GNU C++17                               
*  run_time: 358 ms                                          memory_used: 45700 KB                             
...
int main()
{
  ...
  {
    cr+=a[i];int dl=k-1-i;
     pair<int,int>r=splitbyValue(root,a[i]-1);
    pair<int,int>l=splitbySize(r.second,1);
  ...
  for(int i=0;i<k-pr;i++)prnt.push_back(tc[i].second),m--;
   set<pair<int,int> >st;
  for(int i=pr;i<a.size();i++)st.insert(ta[i]);
