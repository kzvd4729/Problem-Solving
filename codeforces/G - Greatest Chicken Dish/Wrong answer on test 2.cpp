/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/30/2019 23:35                        
*  solution_verdict: Wrong answer on test 2                  language: GNU C++14                               
*  run_time: 14071 ms                                        memory_used: 44300 KB                             
      ...
      else if(nwa>=pra&&nwb<=prb)
      {
        ...
        for(int i=prb;i>nwb;i--)
        {
          for(int j=0;j<rv[i].size();j++)
          {
            ...
            ans[rv[i][j].first]-=rv[i][j].second-max(nwa,rv[i][j+1].second)+1;
          }
        }
      }
      ...
      else if(pra>=nwa&&prb<=nwb)
      {
        ...
        for(int i=nwb;i>prb;i--)
        {
          for(int j=0;j<rv[i].size();j++)
          {
            ...
            ans[rv[i][j].first]+=rv[i][j].second-max(pra,rv[i][j+1].second)+1;
          }
        }
      }
      else assert(0);
