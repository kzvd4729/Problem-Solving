/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/29/2020 14:13                        
*  solution_verdict: Wrong answer on test 4                  language: GNU C++17                               
*  run_time: 77 ms                                           memory_used: 57500 KB                             
...
int sm[40+2][N+2],a[N+2],b[N+2],c[N+2],l[N+2],r[N+2],fr[N+2];
int main()
{
  ...
  for(int i=1;i<(1<<m);i++)
  {
    vector<int>v;int L=0,R=n,cnt=0;
    for(int j=0;j<m;j++)
    {
      if(!(i&(1<<j)))continue;
      if(fr[l[j]]==0)fr[l[j]]=1,v.push_back(l[j]),cnt++;
      if(fr[r[j]]==0)fr[r[j]]=1,v.push_back(r[j]),cnt++;
      L=max(L,a[l[j]]),R=min(R,b[r[j]]);
    }
    L=max(L,cnt);for(auto x:v)fr[x]=0;
    if(L>R)continue;
    //cout<<L<<" (( "<<R<<endl;
    int ad=(sm[cnt][R]-sm[cnt][L-1]+mod)%mod;
