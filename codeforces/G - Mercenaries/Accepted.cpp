/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/29/2020 14:17                        
*  solution_verdict: Accepted                                language: GNU C++17                               
*  run_time: 6801 ms                                         memory_used: 57900 KB                             
...
int main()
{
  ...
  for(int i=1;i<(1<<m);i++)
  {
    ...
    {
      if(!(i&(1<<j)))continue;
      if(fr[l[j]]==0)fr[l[j]]=1,v.push_back(l[j]);
      if(fr[r[j]]==0)fr[r[j]]=1,v.push_back(r[j]);
    }
    int L=v.size(),R=n;
    for(auto x:v)L=max(L,a[x]),R=min(R,b[x]),fr[x]=0;
    if(L>R)continue;
    //cout<<L<<" (( "<<R<<endl;
