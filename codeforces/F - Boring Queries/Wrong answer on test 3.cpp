/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/05/2020 15:52                        
*  solution_verdict: Wrong answer on test 3                  language: GNU C++14                               
*  run_time: 140 ms                                          memory_used: 160200 KB                            
  ...
  for(int i=1;i<=n;i++)
  {
     ...
     for(auto x:fac[a[i]])
    {
      ...
      for(auto z:st[x])
      {
        if(z.second<=nm)
        {
          rot[i]=++sz;upd(rot[i],1,n,ls,i,inv[z.second]);
          ls=rot[i];
        }
        else
        {
          rot[i]=++sz;upd(rot[i],1,n,ls,i,inv[nm]);
