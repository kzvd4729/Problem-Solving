/****************************************************************************************
*  @author: kzvd4729                                         created: Oct/05/2020 15:56                        
*  solution_verdict: Wrong answer on test 3                  language: GNU C++14                               
*  run_time: 124 ms                                          memory_used: 160200 KB                            
  ...
  for(int i=1;i<=n;i++)
  {
     ...
     for(auto x:fac[a[i]])
    {
      ...
      while(a[i]%x==0)cnt++,a[i]/=x,nm=nm*x;
       rot[i]=++sz;upd(rot[i],1,n,ls,i,nm);
      ls=rot[i];vector<pair<int,int> >bf;
      for(auto z:st[x])
      {
        if(z.second<=nm)
        {
          rot[i]=++sz;upd(rot[i],1,n,ls,z.first,inv[z.second]);
          ls=rot[i];
        }
        else
        {
          rot[i]=++sz;upd(rot[i],1,n,ls,z.first,inv[nm]);
          ls=rot[i];bf.push_back({z.first,z.second/nm});
        }
      }
      bf.push_back({i,nm});
      st[x]=bf;
