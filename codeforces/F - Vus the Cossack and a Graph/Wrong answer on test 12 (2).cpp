/****************************************************************************************
*  @author: kzvd4729                                         created: Jun/29/2019 19:13                        
*  solution_verdict: Wrong answer on test 12                 language: GNU C++14                               
*  run_time: 3946 ms                                         memory_used: 156900 KB                            
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    if(dl[i])continue;int nd=i;
    st.erase({adj[nd].size(),nd});
    for(auto x:adj[nd])
    {
  ...
  while(st.size())
  {
    ...
    ed.erase({min(u,v),max(u,v)});
     if(dl[u]==0)
    {
      int nd=u;st.erase({adj[nd].size(),nd});
      for(auto x:adj[nd])
      {
        st.erase({adj[x].size(),x});
        adj[x].erase(nd);
    ...
    if(dl[v]==0)
    {
      int nd=v;st.erase({adj[nd].size(),nd});
      for(auto x:adj[nd])
      {
        st.erase({adj[x].size(),x});
        adj[x].erase(nd);
