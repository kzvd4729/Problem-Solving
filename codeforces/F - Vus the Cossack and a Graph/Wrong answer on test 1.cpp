/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/01/2019 18:27                        
*  solution_verdict: Wrong answer on test 1                  language: GNU C++14                               
*  run_time: 30 ms                                           memory_used: 27500 KB                             
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    ...
    for(int i=0;i<path.size()-1;i++)
    {
      if(i==path.size()-2)
        if(path[1]==0)dl=0;
      if(path[i]==0||path[i+1]==0){dl=0;continue;}
      if(!dl)dl=1;
      else
      {
        ed.erase({min(path[i],path[i+1]),max(path[i],path[i+1])});
        dl=0;
      }
    }
  }
