/****************************************************************************************
*  @author: kzvd4729                                         created: Mar/04/2019 00:20                        
*  solution_verdict: Accepted                                language: GNU C++14                               
*  run_time: 717 ms                                          memory_used: 61200 KB                             
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
    par[i]=i;
  long mst=0,mn=1e18;vector<int>med;int xy=2e9,xyz;
  for(int i=1;i<=m;i++)
  {
    ...
    if(ed[i].lm<=xy)xy=ed[i].lm,xyz=i;
    par[r1]=r2;
    //cout<<ed[i].u<<" ---- "<<ed[i].v<<"\n";
  ...
  for(int i=1;i<=m;i++)
  {
    ...
    //cout<<ed[i].u<<" "<<ed[i].v<<" "<<ret.first<<" "<<ret.second<<"\n";
  ...
  if(add==-1)
  {
    for(auto x:med)
    {
      if(x==xyz)cout<<ed[x].id<<" "<<ed[x].ds-s/ed[x].lm<<"\n";
      else cout<<ed[x].id<<" "<<ed[x].ds<<"\n";
    }
  }
  else
  {
    ...
    for(auto x:med)
    {
      if(x==add)cout<<ed[x].id<<" "<<ed[x].ds-s/ed[x].lm<<"\n";
