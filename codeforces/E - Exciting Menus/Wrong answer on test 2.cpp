/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/31/2019 01:12                        
*  solution_verdict: Wrong answer on test 2                  language: GNU C++14                               
*  run_time: 1840 ms                                         memory_used: 28600 KB                             
...
struct ahoCorasick
{
  ...
  void clean(int n=N)
  {
    for(int i=0;i<=n;i++)
    {
      trie[i].f=0,trie[i].mx=0,trie[i].oc=0;
  ...
  void traverse(int nd,int len)
  {
    for(int i=1;i<=sz;i++)
    {
      long oc=trie[i].oc+trie[i].f;
