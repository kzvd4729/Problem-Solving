/****************************************************************************************
*  @author: JU_Troubleshooer: tariqiitju, the0dd1out, kzvd4729 created: Oct/20/2018 18:55                        
*  solution_verdict: Wrong answer on test 39                 language: GNU C++14                               
*  run_time: 1419 ms                                         memory_used: 3500 KB                              
*  problem: https://codeforces.com/contest/1070/problem/J
****************************************************************************************/
#include <bits/stdc++.h>
using namespace std;
...
int dp(int i,int s)
{
    ...
    if(~res) return res;
    res=dp(i-1,s);
    if(fr[i]<=s  && res < dp(i-1,s-fr[i])+fr[i])
        res=fr[i]+dp(i-1,s-fr[i]);
    return res;
}
int main()
{
     ...
     while(tc--)
    {
        ...
        for(int i=0;i<26;i++)
        {
            ...
            ///i is the devider
            int rem=n-dp(25,n);
            int kuku = k-tmp -dp(25,n);
            kuku=max(0,m-kuku);
            if(tmp >=kuku+rem)
                ans=min(ans,kuku*rem);
            else if(rem==tmp)
                ans=0;
            rem=m-dp(25,m);
            kuku = k-tmp - dp(25,m);
