/****************************************************************************************
*  @author: kzvd4729                                         created: Jul/15/2020 19:20                        
*  solution_verdict: Time limit exceeded on test 7           language: GNU C++17                               
*  run_time: 1000 ms                                         memory_used: 80900 KB                             
...
bool ck(int node,int par)
{
  ...
  //if(mx[node]!=mn[node])return false;
  for(auto x:adj[node])
  {
    if(x==par)continue;
...
{
  //cout<<node<<" ** "<<mxx<<" "<<mnn<<" "<<sz<<endl;
  int f=1;//if(mxx!=mnn)f=0;
  //if(mx[node]!=mxx)f=0;if(mn[node]!=mnn)f=0;
  ...
  if(f)kill(node);if(mnn==mod)mnn=0;
  for(auto x:adj[node])
  {
    if(x==par)continue;
