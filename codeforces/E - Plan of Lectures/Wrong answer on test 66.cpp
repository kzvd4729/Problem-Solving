/****************************************************************************************
*  @author: kzvd4729                                         created: Dec/17/2020 23:13                        
*  solution_verdict: Wrong answer on test 66                 language: GNU C++17 (64)                          
*  run_time: 483 ms                                          memory_used: 61600 KB                             
...
int main()
{
  ...
  for(int i=1;i<=n;i++)if(vs[i]==0&&in[i]==0)topo(i);
   sz=0;
  for(auto x:ord)sz+=(int)rm[x].size();
  if(sz!=n)assert(0);
   for(auto x:ord)for(auto v:rm[x])cout<<v<<" ";
     return 0;
}