/****************************************************************************************
*  @author: kzvd4729                                         created: Jan/18/2018 12:57                        
...
int main()
{
    ...
    {
        cin>>arr[i].s>>arr[i].x;
        if(arr[i].s=="sell")sl[arr[i].x].push_back(i);
    }
    bigint dp[5003];
    for(long long i=0; i<=5000; i++)dp[i]=0;
    pw[1]=2;
    ...
    for(long long i=1; i<=n; i++)
    {
        dp[i-1]>dp[i]?dp[i]=dp[i-1]:dp[i]=dp[i];
        if(arr[i].s=="sell")continue;
        long long xx=arr[i].x;
        long long id=upper_bound(sl[xx].begin(),sl[xx].end(),i)-sl[xx].begin();
        if(id>=sl[xx].size())continue;
        id=sl[xx][id];
        bigint one=dp[id];
        bigint two=dp[i-1]+pw[xx];
        one>two?dp[id]=one:dp[id]=two;
    }
    cout<<dp[n]<<endl;
    return 0;
}