/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/17/2020 21:04                        
*  solution_verdict: Runtime error on test 2                 language: GNU C++17                               
...
int main()
{
  ...
  int n;cin>>n;long tt=0,sm=0;int mn;
  set<int>st,a,b;a.insert(0);
  while(n--)
  {
    int ty,vl;cin>>ty>>vl;
    if(sm==0)mn=inf+1;assert(mn);
    //cout<<tt<<" ** "<<sm<<" "<<mn<<endl;
    if(!ty)
    {
      if(vl>0)
