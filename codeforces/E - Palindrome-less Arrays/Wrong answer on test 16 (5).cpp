/****************************************************************************************
*  @author: kzvd4729                                         created: Jun/06/2020 14:02                        
*  solution_verdict: Wrong answer on test 16                 language: GNU C++14                               
*  run_time: 62 ms                                           memory_used: 5500 KB                              
...
{  
  ...
  if(a==b)
  {
    for(int i=1;i<=sz;i++)dp1[i][0]=-1,dp1[i][1]=-1;
    return dfs1(1,1);
  }
  for(int i=1;i<=sz;i++)dp2[i][0][0]=-1,dp2[i][0][1]=-1,dp2[i][1][0]=-1,dp2[i][1][1]=-1;
  return dfs2(1,1,0);
}
int aa[N+2];
int main()
