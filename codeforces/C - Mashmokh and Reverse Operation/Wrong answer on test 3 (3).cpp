/****************************************************************************************
*  @author: kzvd4729                                         created: Jun/22/2020 18:27                        
*  solution_verdict: Wrong answer on test 3                  language: GNU C++17                               
*  run_time: 77 ms                                           memory_used: 147800 KB                            
...
using namespace std;
const long N=(1<<21)+2;
long aa[N+2];long a[N+2],b[N+2];
vector<long>seg[4*N+2];
long build(long node,long lo,long hi)
{
  ...
  long md=(lo+hi)/2;
  long l=build(node*2,lo,md);build(node*2+1,md+1,hi);
  l++;
...
int main()
{
  ...
  long n;cin>>n;
  for(long i=0;i<(1<<n);i++)cin>>aa[i];
  build(1,0,(1<<n)-1);
   //cout<<seg[2].size()<<" ** "<<seg[3].size()<<endl;
  for(long i=1;i<=n;i++)b[i]-=a[i];
  long q;cin>>q;
  while(q--)
  {
    long x;cin>>x;
    for(long i=1;i<=x;i++)swap(a[i],b[i]);
     long ans=0;
    for(long i=1;i<=n;i++)ans+=a[i];
    cout<<ans<<"\n";
  }
  return 0;
}