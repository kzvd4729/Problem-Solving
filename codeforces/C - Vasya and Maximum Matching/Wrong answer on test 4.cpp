/****************************************************************************************
*  @author: kzvd4729                                         created: Aug/18/2020 21:00                        
...
{
  for(auto x:adj[nd])if(x!=pr)dfs(x,nd);
  if(adj[nd].size()==1&&pr!=-1)
  {
    dp[nd][0]=1;return;
  }
  ...
  for(int i=0;i<n;i++)
  {
    ...
    {
      int now=(1LL*tt*big(dp[adj[nd][i]][0]+dp[adj[nd][i]][1]+dp[adj[nd][i]][3],mod-2))%mod;
      now=(1LL*tt*big(dp[adj[nd][j]][0]+dp[adj[nd][j]][1]+dp[adj[nd][j]][3],mod-2))%mod;
       dp[nd][2]=(dp[nd][2]+((now*dp[adj[nd][i]][1])%mod*dp[adj[nd][j]][1]))%mod;
    }
  }
}
int main()
{
  ...
  dfs(1,-1);cout<<(dp[1][0]+dp[1][1]+dp[1][3])%mod<<endl;
   return 0;
}