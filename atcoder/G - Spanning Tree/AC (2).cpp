/****************************************************************************************
*  @author: kzvd4729                                         created: 2021-05-04 01:23:12                      
*  solution_verdict: AC                                      language: C++ (GCC 9.2.1)                         
*  run_time: 60 ms                                           memory_used: 4548 KB                              
...
int numberOfSpanningTrees(int n)
{
  ...
  for(int i=1;i<=n;i++)
  {
    ...
    for(int j=1;j<=n+1;j++)
    {
      a[i][j]=(1LL*a[i][j]*ml)%mod;
    }
    for(int j=i+1;j<=n;j++)
    {
      vector<int>tmp=a[i];int ji=a[j][i];
      for(int k=1;k<=n+1;k++)
      {
        a[j][k]=(a[j][k]-1LL*tmp[k]*ji)%mod;
        a[j][k]=(a[j][k]+mod)%mod;
      }
    }
  }
