/****************************************************************************************
*  @author: kzvd4729                                         created: 2022-02-06 01:06:06                      
*  solution_verdict: TLE                                     language: C++ (GCC 9.2.1)                         
*  run_time: 3311 ms                                         memory_used: 104940 KB                            
 ...
 void PreCal()
{
    ...
    for(i=2;i<=N/2;i+=2)
    {
        ...
        while(j%2 == 0)j/=2, primeDivs[i].back().second++;
    }
    for(i=3;i<=N/2;i+=2)
    {
        if(mark[i])continue;
        for(j=i;j<=N/2;j+=i)
        {
            ...
            while(k%i == 0)k/=i, primeDivs[j].back().second++;
        }
    }
}
  ///Calculation
