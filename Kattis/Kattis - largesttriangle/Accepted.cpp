/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-10-01 17:20:20                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 330                                        memory_used (MB):                                 
...
int main()  
{
  ...
  for(int i=0;i<n;i++)
  {
    ...
    for(int j=i+2;j<n;j++)
    {
      ...
      while(true)
      {
        double d=l.pointDist(h[pt]);
        double d1=l.pointDist(h[(pt+1)%n]);
        double d2=l.pointDist(h[(pt-1+n)%n]);

        if(d1>d&&d1>d2)pt++;
        else if(d2>d&&d2>d1)pt--;
        else 
        {
