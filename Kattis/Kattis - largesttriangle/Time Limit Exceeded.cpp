/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-10-01 17:09:57                      
*  solution_verdict: Time Limit Exceeded                     language: C++                                     
*  run_time (ms):                                            memory_used (MB):                                 
*  problem: https://vjudge.net/problem/Kattis-largesttriangle
****************************************************************************************/
#include<iostream>
#include<vector>
#include<cstring>
...
int main()  
{
  ...
  int n;cin>>n;
  polygon p;p.p.resize(n);
  for(int i=0;i<n;i++)cin>>p.p[i].x>>p.p[i].y;

  p.convexHull();vector<point>h=p.h;

  double ans=0;n=h.size();
  if(h.size()<3)cout<<ans<<endl,exit(0);
  for(int i=0;i<n;i++)
  {
    for(int j=i+1;j<n;j++)
    {
      line l;l.makeLine(h[i],h[j]);

      int lo=i+1,hi=j-1,md1,md2;
      while(hi-lo>2)
      {
        md1=(lo+lo+hi)/3;
        double d1=l.pointDist(h[md1]);

        md2=(lo+hi+hi)/3;
        double d2=l.pointDist(h[md2]);

        if(d1>d2)hi=md2;else lo=md1;
      }
      double d=0;
      for(int md=lo;md<=hi;md++)d=max(d,l.pointDist(h[md]));

      lo=j+1,hi=n+i-1,md1,md2;
      while(hi-lo>2)
      {
        md1=(lo+lo+hi)/3;
        double d1=l.pointDist(h[md1%n]);

        md2=(lo+hi+hi)/3;
        double d2=l.pointDist(h[md2%n]);

        if(d1>d2)hi=md2;else lo=md1;
      }
      for(int md=lo;md<=hi;md++)d=max(d,l.pointDist(h[md%n]));

      ans=max(ans,h[i].dist(h[j])*d);
    }
  }
  cout<<ans*0.5<<endl;
  return 0;
