/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-09-29 14:15:29                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 750                                        memory_used (MB):                                 
*  problem: https://vjudge.net/problem/Kattis-dogs
****************************************************************************************/
#include<iostream>
#include<vector>
#include<cstring>
...
int main()  
{
  ...
  int p=2,q=2;double ans=1e9;
  point x=a[1],y=b[1];
  while(p<=m&&q<=n)
  {
    double d1=x.dist(a[p]);
    double d2=y.dist(b[q]);

    point p1,p2;
    double lo=0,hi=min(d1,d2),md1,md2;
    for(int i=1;i<=100;i++)
    {
      md1=(lo+lo+hi)/3.0;
      p1=cal(x,a[p],md1);
      p2=cal(y,b[q],md1);
      double ds1=p1.dist(p2);

      md2=(lo+hi+hi)/3.0;
      p1=cal(x,a[p],md2);
      p2=cal(y,b[q],md2);
      double ds2=p1.dist(p2);


      ans=min(ans,ds1);ans=min(ans,ds2);
      if(ds1<ds2)hi=md2;else lo=md1;

    }    
    if(d1<d2){x=a[p++],y=cal(y,b[q],d1);}
    else {y=b[q++],x=cal(x,a[p],d2);}
  }
  cout<<ans<<endl;
  return 0;
}