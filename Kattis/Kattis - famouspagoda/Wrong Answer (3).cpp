/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-02-12 22:00:02                      
...
const int N=1e6;
const long inf=1e18;
int mx[2000+2][2000+2],mn[2000+2][2000+2];
...
void case2(int n,int g)
{
  ...
  for(int i=1;i<=n;i++)
  {
    for(int j=i;j>=1;j--)
    {
      int lo=mn[i][j],hi=mx[i][j],md1,md2;
      while(hi-lo>2)
      {
...
void case1(int n,int g)
{
  ...
  for(int i=1;i<=n;i++)
  {
    for(int j=i;j>=1;j--)
    {
      ...
      for(int md=lo;md<=hi;md++)
      {
        mt[j][i]=min(mt[j][i],ck1(j,i,md)*1LL);
      }
    }
  }
} 
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    mx[i][i]=aa[i],mn[i][i]=aa[i];
    for(int j=i+1;j<=n;j++)
    {
      mx[i][j]=max(mx[i][j-1],(int)aa[j]);
      mn[i][j]=min(mn[i][j-1],(int)aa[j]);
    }
  }
