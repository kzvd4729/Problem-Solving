...
*  run_time: 6 ms                                            memory_used: 668 KB                               
*  problem: 
****************************************************************************************/
#include<bits/stdc++.h>
#define long long long
using namespace std;
const long con=10e10;
long n,x,y,d,v,dd,vv,id;
vector<pair<double,long> >vc,un,up,dw;
long bs1(void)
{
  double x=((dd*1.00)/(vv*1.00));
  long lo,hi,md;
  lo=0,hi=n-1;
  while(hi-lo>2)
  {
...
int main()
{
  ...
  for(int i=1;i<=n;i++)
  {
    ...
    long vx=v+x;
    long vy=v-y;
    double x=((d*1.00)/(v*1.00));
    double y=((d*1.00)/(vx*1.00));
    double z=((d*1.00)/(vy*1.00));
    un.push_back({x,i});
    up.push_back({y,i});
    dw.push_back({z,i});
  }
