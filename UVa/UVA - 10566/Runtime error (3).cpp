/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-05-26 21:44:58                      
...
struct line
{
  ...
  {
    if(isParallel(l))return point(0,0);
...
int main()
{
  ...
  double x,y,c;int t,tc=0;
  while(cin>>x>>y>>c)
  {
    
    double lo=0.00000001,hi=min(x,y)-0.000001,md;
    int loop=100;
    while(loop--)
    {
      md=(lo+hi)/2;
    ...
    cout<<setprecision(3)<<fixed<<md+1<<endl;
  }
  return 0;
}