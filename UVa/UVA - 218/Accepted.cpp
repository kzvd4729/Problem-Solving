/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-07-25 01:01:04                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time: 10                                              memory_used:                                      
*  problem: https://vjudge.net/problem/UVA-218
****************************************************************************************/
#include<bits/stdc++.h>
#define long long long
using namespace std;
const int N=1e6;
struct point
{
  double x,y;
  bool operator<(const point &p)const{return y<p.y||(y==p.y&&x<p.x);}
  point operator-(const point &p)const{return {x-p.x,y-p.y};}
  double operator^(const point &p)const{return x*p.y-y*p.x;}
  double dis(const point &p)const{return sqrt((x-p.x)*(x-p.x)+(y-p.y)*(y-p.y));}
};
point stck[N+2];int top=0;
void balance(void)
{
  while(top>=3)
  {
    //>=0 to omit straight line point;
    if(((stck[top-1]-stck[top-2])^(stck[top]-stck[top-2]))>=0)
      stck[top-1]=stck[top],top--;
    else break;
  }
}
//returns counter-clock-wise;
//first=last;
//is it ok for repetative point?
//may be handle 1 case differently
vector<point>monotoneChain(vector<point>&v)
{
  sort(v.begin(),v.end());
  vector<point>hull;int n=v.size();
  for(int i=n-1;i>=0;i--)
    stck[++top]=v[i],balance();
  while(top>0)
    hull.push_back(stck[top--]);
  for(int i=0;i<n;i++)
    stck[++top]=v[i],balance();
  top--;
  while(top>0)
    hull.push_back(stck[top--]);
  return hull;
}
int main()
{
  ios_base::sync_with_stdio(0);cin.tie(0);
  int n,tc=0;
  while(cin>>n)
  {
    if(!n)break;vector<point>v(n);
    for(int i=0;i<n;i++)
      cin>>v[i].x>>v[i].y;
    v=monotoneChain(v);
    double pr=0;
    for(int i=1;i<v.size();i++)
      pr+=v[i].dis(v[i-1]);
    v.pop_back();
    reverse(v.begin(),v.end());
    v.push_back(v[0]);
    if(tc)cout<<"\n";
    cout<<"Region #"<<++tc<<":\n";
    int f=0;
    for(auto x:v)
    {
      if(f)cout<<"-";f=1;
      cout<<setprecision(1)<<fixed<<"("<<x.x<<","<<x.y<<")";
    }
    cout<<"\n";
    cout<<"Perimeter length = "<<setprecision(2)<<fixed<<pr<<"\n";
  }
  return 0;
}