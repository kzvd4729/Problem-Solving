/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-03-02 17:50:48                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time: 0                                               memory_used:                                      
*  problem: https://vjudge.net/problem/UVA-10652
****************************************************************************************/
#include<bits/stdc++.h>
...
int main()
{
  ...
  while(t--)
  {
    int n;cin>>n;polygon pl;double ar=0;
    for(int i=1;i<=n;i++)
    {
      point c;cin>>c.x>>c.y;double w,h,ang;cin>>w>>h>>ang;
      if(ang<0)ang+=360;ang=degToRed(ang);
      ang=pi+pi-ang;ar+=w*h;

      point p;
      p=point(w/2.0,h/2.0);pl.p.push_back(c+p.rotateCCW(ang));
      p=point(w/2.0,-h/2.0);pl.p.push_back(c+p.rotateCCW(ang));
      p=point(-w/2.0,h/2.0);pl.p.push_back(c+p.rotateCCW(ang));
      p=point(-w/2.0,-h/2.0);pl.p.push_back(c+p.rotateCCW(ang));
    }
    double area=pl.hullArea();
    double pr=(ar/area)*100;
    cout<<setprecision(1)<<fixed<<pr<<" %"<<endl;
  }
  return 0;
}