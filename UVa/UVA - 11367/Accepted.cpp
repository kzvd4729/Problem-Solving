/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-04-04 00:03:22                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time: 1610                                            memory_used:                                      
...
const int N=1e3;
const int inf=1e9;
int aa[N+2],n,m,cp,st,ed,ds[N+2][102],qd[N+2][102];
  ...
  while(cin>>n>>m)
  {
    ...
    while(q--)
    {
      ...
      while(q.size())
      {
        ...
        int nd=p.first;qd[p.first][p.second]=0;
        for(auto x:adj[nd])
        {
          ...
          if(ds[x.first][p.second-x.second]>ds[nd][p.second])
          {
            ...
            if(!qd[x.first][p.second-x.second])
            {
              q.push({x.first,p.second-x.second});
              qd[x.first][p.second-x.second];
            }
            for(int i=1; ;i++)
            {
              ...
              if(ds[x.first][p.second-x.second+i]>ds[nd][p.second]+i*aa[x.first])
              {
                ...
                if(!qd[x.first][p.second-x.second+i])
                {
                  q.push({x.first,p.second-x.second+i});
                  qd[x.first][p.second-x.second+i]=1;
                }
              }
            }
          }
