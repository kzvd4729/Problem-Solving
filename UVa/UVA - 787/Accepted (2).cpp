/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-02-27 18:52:59                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time: 1090                                            memory_used:                                      
*  problem: https://vjudge.net/problem/UVA-787
****************************************************************************************/
#include<bits/stdc++.h>
    ...
    for(int i=1;i<=id+1;i++)
    {
      if(aa[i]=="0")
      {
        if(ng%2==0)
        {
          ...
          for(int j=lt;j<i;j++)
            ans=Mul(ans,Abs(aa[j])),f=1;
          if(Compare(ans,mx)==1&&f)
            mx=ans;
        }
        else
        {
          string ans="1";int tmp=ng,f=0;
          for(int j=lt;j<i;j++)
          {
            if(aa[j][0]=='-')tmp--;
            if(tmp==0)break;
