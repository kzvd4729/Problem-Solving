/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-04-30 20:53:48                      
...
void dfs(long node,long par)
{
  ...
  long x=nx[node].back();
  tmp[sz-1][1]=dp[x][2];tmp[sz-1][0]=dp[x][0];
  for(long i=sz-2;i>=0;i--)
  {
    x=nx[node][i];
    tmp[i][1]=min(tmp[i+1][1]+dp[x][0],dp[x][2]+tmp[i+1][0]);
    tmp[i][0]=min(dp[x][0]+tmp[i+1][0],tmp[i][1]);
  }
