/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-04-30 19:45:48                      
...
int n,a,b,dp[N+2][3],tmp[N+2][2];
vector<int>adj[N+2],nx[N+2];
void dfs(int node,int par)
{
  ...
  dp[node][0]=0;dp[node][1]=a;dp[node][2]=b;
  if(nx[node].size()==0)return ;
  for(auto x:nx[node])
    dp[node][0]+=dp[x][1];
  for(auto x:nx[node])
    dp[node][1]+=min(dp[x][1],dp[x][0]);
  for(int i=0;i<nx[node].size();i++)
  ...
  for(int i=nx[node].size()-2;i>=0;i--)
  {
    x=nx[node][i];
...
int main()
{
  ...
  {
    if(!(n+a+b))break;
    for(int i=1;i<=n;i++)adj[i].clear(),nx[i].clear();
    for(int i=1;i<n;i++)
    {
      int u,v;cin>>u>>v;
      adj[u].push_back(v);
    ...
    dfs(1,-1);cout<<min(dp[1][0],min(dp[1][1],dp[1][2]))<<"\n";
  }
  return 0;
}