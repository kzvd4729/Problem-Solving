/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-04-30 19:59:17                      
...
void dfs(long node,long par)
{
  ...
  for(auto x:nx[node])
    dp[node][0]+=min(dp[x][1],dp[x][2]);
  for(auto x:nx[node])
    dp[node][1]+=min(dp[x][2],min(dp[x][1],dp[x][0]));
  ...
  {
    x=nx[node][i];
    tmp[i][1]=tmp[i+1][1]+min(dp[x][2],min(dp[x][0],dp[x][1]));
    tmp[i][1]=min(tmp[i][1],dp[x][2]+tmp[i+1][0]);
    tmp[i][0]=min(dp[x][2],min(dp[x][0],dp[x][1]))+tmp[i+1][0];
  ...
  for(auto x:nx[node])
    for(auto z:nx[x])
      dp[node][2]+=min(dp[z][2],min(dp[z][1],dp[z][0]));
  dp[node][1]=min(dp[node][1],dp[node][2]);
