/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-03-27 18:39:37                      
...
int main()
{
  ...
  while(cin>>a.x>>a.y>>b.x>>b.y>>c.x>>c.y>>d.x>>d.y)
  {
    ...
    {
      equation e=parallel_with(make_equation(a,c),b);
      point md;md={(b.x+c.x)/2.0,(b.y+c.y)/2.0};
      equation e2=make_equation(a,md);
      p=solve(e,e2);
    }
    else if(isequal(b,d))
    {
      equation e=parallel_with(make_equation(b,c),a);
      point md;md={(a.x+c.x)/2.0,(a.y+c.y)/2.0};
      equation e2=make_equation(b,md);
      p=solve(e,e2);
    }
