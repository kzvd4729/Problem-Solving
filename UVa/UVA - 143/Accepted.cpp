/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-08-06 14:20:31                      
...
int main()
{
  ...
  while(true)
  {
    ...
    if(sgn(a.x)==0&&sgn(a.y)==0&&sgn(b.x)==0&&sgn(b.y)==0&&sgn(c.x)==0&&sgn(c.y)==0)
      break;
    double xmin=min(ceil(a.x),min(ceil(b.x),ceil(c.x)));
    double ymin=min(ceil(a.y),min(ceil(b.y),ceil(c.y)));

    double xmax=max(floor(a.x),max(floor(b.x),floor(c.x)));
    double ymax=max(floor(a.y),max(floor(b.y),floor(c.y)));
    xmin=max(1.0,xmin);ymin=max(1.0,ymin);
    xmax=min(99.0,xmax);ymax=min(99.0,ymax);
