/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-04-09 19:05:44                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 2340                                       memory_used (MB): 21                              
...
void tooLazy(int node,int lo,int hi)
{
  ...
  if(lo!=hi)
  {
    if(lz[node*2]==-1)lz[node*2]=lz[node];
    else lz[node*2]=min(lz[node*2],lz[node]);
    
    if(lz[node*2+1]==-1)lz[node*2+1]=lz[node];
    else lz[node*2+1]=min(lz[node*2+1],lz[node]);
  }
  lz[node]=-1;
}
