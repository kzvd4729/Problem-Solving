/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-02-28 14:21:07                      
...
void upd(int node,int lo,int hi,int lt,int rt,int v,int k)
{
  too_lazy(node,lo,hi);
  ...
  seg[node].smv=seg[node*2].smv+seg[node*2+1].smv;
  seg[node].smk=seg[node*2].smk+seg[node*2+1].smk;
}
...
int main()
{
  ...
  while(e--)
  {
    ...
    if(c=='U')
    {
      ...
      int ds=Lev[t];
      upd(1,1,n,Ent[t],Ext[t],-ds*k,0);
    }
    else
    {
      int a,b,lc,d;cin>>a>>b;
      lc=Lca(a,b,d);
      long sm=Hqry(a,lc,n);
