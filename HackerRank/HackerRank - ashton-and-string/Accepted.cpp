/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-08-20 11:11:56                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 590                                        memory_used (MB):                                 
*  problem: https://vjudge.net/problem/HackerRank-ashton-and-string
****************************************************************************************/
#include<iostream>
#include<vector>
#include<cstring>
...
int main()
{
  ...
  while(t--)
  {
    string s;cin>>s;long k;cin>>k;int n=s.size();
    ...
    for(int i=1;i<=n;i++)
    {
      long ln=n-sa.SA[i],mn=sa.lcp[i];
      long tt=(ln*(ln+1))/2-(mn*(mn+1))/2;

      if(tt>=k)
      {
        for(int l=mn+1;l<=ln;l++)
        {
          if(l>=k)
          {
            cout<<s[sa.SA[i]+k-1]<<'\n';
