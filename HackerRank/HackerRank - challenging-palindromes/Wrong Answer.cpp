/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-09-03 01:00:10                      
...
void cal(string a,string b)
{
  ...
  {
    if(sa.SA[i]<=n)mn=sa.lcp[i+1];
    else
    {
      mn=min(mn,sa.lcp[i]);
      mt[sa.SA[i]-(n+1)]=max(mt[sa.SA[i]-(n+1)],mn);
    }
  }
  for(int i=n+n+1;i>=1;i--)
  {
    if(sa.SA[i]<=n)mn=sa.lcp[i];
    else
    {
      mt[sa.SA[i]-(n+1)]=max(mt[sa.SA[i]-(n+1)],mn);
      mn=min(mn,sa.lcp[i]);
  ...
  for(int i=0;i<n;i++)mt[i]=min(mt[i],n);
  ...
  for(int i=0;i<n;i++)
  {
    ...
    if(pl+mt[i]+mt[i]>=ln&&mt[i])
    {
      ...
      if(pl+mt[i]+mt[i]>ln)ln=pl+mt[i]+mt[i],ans=now;
