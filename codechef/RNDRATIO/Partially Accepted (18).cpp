/****************************************************************************************
*  @author: kzvd4729                                         created: 24-04-2020 18:56:06                      
*  solution_verdict: Partially Accepted                      language: C++14                                   
*  run_time: 4.54 sec                                        memory_used: 50.7M                                
...
int main()
{
  ...
  while(t--)
  {
    ...
    int n;cin>>n;long tt=1;int mx=N;
    for(int i=1;i<=n;i++)
    {
      cin>>l[i]>>r[i];if(l[i]>r[i])assert(0);
      //if(t==1)l[i]=rand()%10+1;
      //if(t==1)r[i]=100000-rand()%10;
      mx=min(mx,r[i]);
      tt=(tt*(r[i]-l[i]+1))%mod;
    }
    for(int i=1;i<=mx;i++)mul[i]=big(i,n),dp[i]=1;
    int mm;
    for(int i=1;i<=n;i++)
    {
      for(int g=1;g<1800;g++)
        mul[g]=(mul[g]*(getSum(l[i],r[i],g)%mod))%mod;
      int ls=1800;
      while(ls<=mx)
      {
    ...
    for(int i=1;i<=mx;i++)
    {
      int now=(mul[i]*big(i,mod-2))%mod;
      //now=(1LL*now*cnt[i])%mod;
      ex=(ex+now)%mod;
    }
    ex=(ex+mod)%mod;
    cout<<(ex*big(tt,mod-2))%mod<<'\n';
  }
  return 0;
}