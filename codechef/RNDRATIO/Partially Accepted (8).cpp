/****************************************************************************************
*  @author: kzvd4729                                         created: 24-04-2020 18:11:34                      
*  solution_verdict: Partially Accepted                      language: C++14                                   
*  run_time: 4.86 sec                                        memory_used: 28.5M                                
  ...
  while(t--)
  {
    ...
    for(int i=1;i<=n;i++)
    {
      ...
      while(ls<=N)
      {
        ...
        lo=md;
        dp[ls]=(dp[ls]*(in)%mod)%mod;
        if(in>=0&&in<=1000000)dp[lo]=(dp[lo]*inv[in])%mod;
        else dp[lo]=(dp[lo]*big(in%mod,mod-2))%mod;
        ls=lo;
      }
    }
    dp[0]=1;
