/****************************************************************************************
*  @author: kzvd4729                                         created: 25-07-2019 16:34:04                      
*  solution_verdict: Partially Accepted                      language: C++14                                   
*  run_time: 4.41 sec                                        memory_used: 242.1M                               
...
long rotatingCalipers(vector<point>p)
{
  ...
  int n=p.size(),j=1;long ans=0;
  for(int i=0;i<n;i++)
  {
    while(abs((p[(i+1)%n]-p[(j+1)%n])^(p[i]-p[(j+1)%n]))>abs((p[(i+1)%n]-p[j])^
        (p[i]-p[j])))
      j=(j+1)%n;
