/****************************************************************************************
*  @author: kzvd4729                                         created: 25-07-2019 16:58:26                      
*  solution_verdict: Partially Accepted                      language: C++14                                   
*  run_time: 4.33 sec                                        memory_used: 242.1M                               
...
/*long rotatingCalipers(vector<point>p)
{
  ...
  {
    while(p[i].dist(p[j+1])>p[i].dist(p[j]))
    {
      cout<<"here"<<endl;
      j=(j+1)%n;
    }
    ans=max(ans,p[i].dist(p[j]));
    ans=max(ans,p[i+1].dist(p[j+1]));
  }
  return ans;
}*/
//polygoan is convex and ordered
