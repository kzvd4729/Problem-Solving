/****************************************************************************************
*  @author: kzvd4729                                         created: 03-08-2018 16:22:54                      
...
long t,n,a,b;
long mod=1e9+7;
bigint bigmod(bigint b,bigint p,bigint mod)
{
  bigint ret=1;
  while(p!=0)
  {
...
long _bigmod(long b,long p,long mod)
{
  long ret=1;
  while(p!=0)
  {
    if(p&1)ret=((ret)%mod*(b%mod))%mod;
    b=((b%mod)*(b%mod))%mod;
    p/=2;
  }
  return ret;
...
int main()
{
  ...
  while(t--)
  {
    ...
    if(df==0)
    {
      cout<<(_bigmod(a,n,mod)+_bigmod(b,n,mod))%mod<<endl;
      continue;
    }
    long tmp;
    if(df<=mod)tmp=(_bigmod(a,n,df)+_bigmod(b,n,df))%df;
    else tmp=(bigmod(a,n,df)+bigmod(b,n,df))%df;
    if(tmp==0)
    {
      cout<<df<<endl;
      continue;
