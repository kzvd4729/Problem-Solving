/****************************************************************************************
*  @author: kzvd4729                                         created: 25-07-2019 23:42:32                      
*  solution_verdict: Wrong Answer                            language: C++14                                   
*  run_time: 0.46 sec                                        memory_used: 52.1M                                
...
long rotatingCalipers(vector<point>&p)
{
  ...
  if(p.size()==2)return p[0].distSqure(p[1]);
  int n=p.size()-1;long ans=0;a=-1,b=-1;
  for(int i=0,j=1;i<n;i++)
  {
    while(abs((p[i+1]-p[j+1])^(p[i]-p[j+1]))>abs((p[i+1]-p[j])^(p[i]-p[j])))
      j=(j+1)%n;
    long aa=p[j].distSqure(p[i]);
    long bb=p[j].distSqure(p[i+1]);
    if(aa>ans)ans=aa,a=p[i].id,b=p[j].id;
    if(bb>ans)ans=bb,a=p[i+1].id,b=p[j].id;
  }
  return ans;
}
int main()
