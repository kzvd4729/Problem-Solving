/****************************************************************************************
*  @author: kzvd4729                                         created: 03-11-2018 00:00:33                      
*  solution_verdict: Partially Accepted                      language: C++14                                   
*  run_time: 0.22 sec                                        memory_used: 76.1M                                
  ...
  while(t--)
  {
    ...
    for(int i=1;i<=n;i++)
    {
      ...
      for(int j=0;j<=n;j++)
      {
        ...
        {
          b=(b+ncr[n-k][j])%mod;
          bb=(qm[n-k][j]-qm[n-k-1][j]+mod)%mod;
        }
        //long bb=(qm[n-(i+1)][j]-qm[n-(rgt[i]+1)][j]+mod+mod)%mod;
        if(b!=bb)assert(0);
        ans=(ans+a*b)%mod;
      }
    }
