/****************************************************************************************
*  @author: kzvd4729                                         created: 09-04-2021 03:27:07                      
*  solution_verdict: Partially Accepted                      language: C++14                                   
*  run_time: 1.77 sec                                        memory_used: 153.8M                               
...
vector<long>ad[N+2],an[N+2];
long a[N+2],b[N+2],pw[N+2],ls[N+2],lv[N+2],par[N+2],ent[N+2],ext[N+2],tim;
set<vector<long> >st;
void dfs(long nd,long pr,long hs,long l)
{
  ent[nd]=++tim;
  st.insert({hs,l,nd}); long er=hs; lv[nd]=l; par[nd]=pr;
  hs+=pw[a[nd]]-pw[b[nd]];
...
long vs[N+2],dp[N+2];
void rec(long nd,long pr)
{
  for(auto x:ad[nd])if(x!=pr)rec(x,nd);
  dp[nd]=0;
  if((long)an[nd].size()==0)return ;
  long ml=1,star=an[nd][0]; if((long)an[nd].size()>1)assert(0);
  if(star==nd)
  {
    long ch=0;
    for(auto x:ad[nd])
...
int main()
{
  ...
  pw[0]=1;
  for(long i=1;i<=N;i++)pw[i]=(1LL*pw[i-1]*10000019)%mod;
  long t;cin>>t;
  while(t--)
  {
    long n,s;cin>>n>>s;tim=0;
    ...
    st.clear();for(long i=1;i<=n;i++)ls[i]=-1;
    dfs(1,-1,0,1);
    //for(long i=1;i<=n;i++)cout<<i<<" "<<ls[i]<<endl;
    vector<long>v;for(long i=1;i<=n;i++)v.push_back(i);
    sort(v.begin(),v.end(),[&](long a,long b){
      return lv[a]>lv[b];
    });
    for(long i=1;i<=n;i++)vs[i]=0;long f=1;
    for(long i=1;i<=n;i++)an[i].clear();
    for(auto x:v)
    {
      if(vs[x])continue;
      if(ls[x]==-1){f=0;break;}
