/****************************************************************************************
*  @author: kzvd4729                                         created: 14-12-2018 21:23:37                      
*  solution_verdict: Accepted                                language: C++14                                   
*  run_time: 0.78 sec                                        memory_used: 123.6M                               
...
int main()
{
  ...
  for(auto x:ed)
  {
    ...
    sz[x.first]++;avl.insert(x.second);
  }
  for(int i=1;i<=n;i++)
  {
    pq.insert({sz[i],i});
  ...
  while(pq.size())
  {
    ...
    else
    {
      ...
      if(avl.size()==1)
      {
        ans.push_back({1,*avl.begin()});
        del(*avl.begin());
      }
      else
      {
        int a=*avl.begin();avl.erase(a);
        int b=*avl.begin();avl.insert(a);
        del(a);del(b);ans.push_back({2,a,b});
      }
    }
  }
