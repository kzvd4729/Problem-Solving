/****************************************************************************************
*  @author: kzvd4729                                         created: 12-06-2021 22:41:21                      
*  solution_verdict: Time Limit Exceeded                     language: C++14                                   
*  run_time: 1.01 sec                                        memory_used: 656M                                 
...
int xx,yy;
gp_hash_table<int,long long>bit[N+2],bit2[N+2];
...
long long get(int x,int y)
{
  ...
  {
    int z=y;
    for( ;z>0;z-=z&-z)if(bit[x].find(z)!=bit[x].end())ret+=bit[x][z];
  }
  return ret;
}
...
long long get2(int x,int y)
{
  ...
  {
    int z=y;
    for( ;z<=yy;z+=z&-z)if(bit2[x].find(z)!=bit2[x].end())ret+=bit2[x][z];
  }
  return ret;
}
