/****************************************************************************************
*  @author: kzvd4729                                         created: 2018-08-06 14:08:32                      
...
double rotating_calipars(void)
{
  ...
  int m=hull.size();
  int k=1;
  while(fabs(area(hull[m-1],hull[0],hull[(k+1)%m]))>fabs(area(hull[m-1],hull[0],hull[k])))
    k++;
  for(int i=0,j=k;i<=k&&j<m;i++)
  {
    ret=max(ret,dist(hull[i],hull[j]));
    while(j<m&&fabs(area(hull[i],hull[(i+1)%m],hull[(j+1)%m]))>fabs(area(hull[i],hull[(i+1)%m],hull[j])))
    {
