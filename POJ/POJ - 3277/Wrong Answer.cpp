/****************************************************************************************
*  @author: kzvd4729                                         created: 2018-08-06 17:53:32                      
...
void upd(long node,long lo,long hi,long lt,long rt)
{
...
long query(long node,long lo,long hi,long lt,long rt)
{
  ...
  long md=(lo+hi)/2;
  long p1=query(seg[node].l,lo,md,lt,rt);
  long p2=query(seg[node].r,md+1,hi,lt,rt);
  return p1+p2;
}
...
int main()
{
  ...
  for(long i=1;i<=n;i++)
  {
    long tmp=query(1,1,R,arr[i].l,arr[i].r-1);
