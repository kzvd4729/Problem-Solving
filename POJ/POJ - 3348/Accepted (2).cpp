/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-02-09 17:53:28                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 125                                        memory_used (MB): 0.7                             
...
struct polygoan
{
  ...
  */
  vector<point>convexHull()
  {
    ...
    return hull;//returns counter-clock-wise,non empty,a path,not closed circuit
  }
  double hullArea()
  {
    vector<point>h=convexHull();double ret=0;
    int n=h.size();//cout<<n<<endl;
    for(int i=1;i<n;i++)ret+=(h[i]-h[0])^(h[(i+1)%n]-h[0]);
    return ret/2.0;//signed, positive for cw
  }
};
