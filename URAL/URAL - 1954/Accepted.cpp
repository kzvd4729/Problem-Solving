/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-09-05 20:19:25                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 46                                         memory_used (MB): 39.9                            
...
void init()
{
  ...
  for(int i=0;i<N+2;i++)
    pal[i].tmp={N+2,N+2},memset(pal[i].next,0,sizeof pal[i].next);
...
int main()
{
  ...
  while(t--)
  {
    ...
    manacher(s);
    //for(int i=1;i<=s.size();i++)cout<<dp[i].first<<" "<<dp[i].second<<endl;
    int n=s.size();
    ...
    else
    {
      ...
      assert((int)v.size()==5);int tt=0;
      string tmp;
      for(auto x:v)
      {
        tt+=x.second-x.first+1;
        assert(ck(x.first,x.second));
        tmp+=s.substr(x.first-1,x.second-x.first+1);
        cout<<s.substr(x.first-1,x.second-x.first+1)<<endl;
      }
      assert(tmp==s);assert(tt==n);
    }
    v.clear();od.clear();ev.clear();
  }
  return 0;
}