/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-09-05 21:13:33                      
*  solution_verdict: Runtime error (access violation)        language: C++                                     
...
int main()
{
  ...
  {
    long msk=0;
    for(int j=dp[i].first;j<=min(31,i);j+=2)msk|=(1LL<<(j-1));
    for(int j=dp[i].second;j<=min(31,i);j+=2)msk|=(1LL<<(j-1));
    cout<<msk<<'\n';
  }
  return 0;
}