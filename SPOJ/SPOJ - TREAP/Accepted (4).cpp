/****************************************************************************************
*  @author: kzvd4729                                         created: 2018-09-28 17:22:16                      
*  solution_verdict: Accepted                                language: C++                                     
*  run_time (ms): 7470                                       memory_used (MB): 129                             
...
void _cal(int node)
{
  ...
  if(st[node].mx!=st[node].mn)st[node].mxg=st[node].mx-st[node].mn;
}
...
int main()
{
  ...
  while(n--)
  {
    ...
    if(ch=='I')
    {
      int x;cin>>x;if(mp[x])continue;mp[x]=1;
    ...
    {
      int x;cin>>x;if(!mp[x])continue;mp[x]=0;
      pair<int,int>p1=split_vl(root,x);
