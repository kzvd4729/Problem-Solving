/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-10-07 22:58:26                      
...
void add(int p,int nm,int ds)
{
  if(st[p].size()>=2)
  {
    auto it=st[p].rbegin();it--;
    ov.erase(ov.find(*st[p].rbegin()+*it));
  }
  if(s[p][nm].size())
  {
  ...
  {
    auto it=st[p].rbegin();it--;
    ov.insert(*st[p].rbegin()+*it);
  }
}
void del(int p,int nm,int ds)
{
  if(st[p].size()>=2)
  {
    auto it=st[p].rbegin();it--;
