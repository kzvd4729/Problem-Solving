/****************************************************************************************
*  @author: kzvd4729                                         created: 2019-08-21 15:37:56                      
...
using namespace std;
const long N=1e6,b=617,mod1=1000001011,mod2=1000009999;
long tr,pw1[N+2],pw2[N+2];
struct treap
{
  long lt,rt,dt,pri,sz;
  long hs1,hs2;
}bst[N+2];
void cal(long node)
{
  long l=bst[node].lt,r=bst[node].rt;
...
int main()
{
  ...
  pw1[0]=1;
  for(long i=1;i<=N;i++)pw1[i]=(1LL*pw1[i-1]*b)%mod1;
  pw2[0]=1;
  for(long i=1;i<=N;i++)pw2[i]=(1LL*pw2[i-1]*b)%mod2;

  long t;cin>>t;
  while(t--)
  {
    string s;cin>>s;long root=0,n=s.size();tr=0;
    for(auto x:s)
    {
      bst[++tr]={0,0,(long)x,rand(),1,(long)x,(long)x};
      root=join(root,tr);
    }
    long q;cin>>q;
    while(q--)
    {
      ...
      {
        long l,r;cin>>l>>r;
        long lo=1,hi=min(n-r,n-l)+1,md;
        while(hi-lo>2)
        {
          md=(lo+hi)/2;
          pair<long,long>p=splitbySize(root,l-1);
          pair<long,long>q=splitbySize(p.second,md);
          long a=bst[q.first].hs1,b=bst[q.first].hs2;
          root=join(q.first,q.second);
          root=join(p.first,root);

          p=splitbySize(root,r-1);
          q=splitbySize(p.second,md);
          long c=bst[q.first].hs1,d=bst[q.first].hs2;
          root=join(q.first,q.second);
          root=join(p.first,root);
      ...
      {
        long x;cin>>x>>c;
        pair<long,long>p=splitbySize(root,x-1);
        pair<long,long>q=splitbySize(p.second,1);
        bst[++tr]={0,0,(long)c,rand(),1,(long)c,(long)c};
