/****************************************************************************************
*  @author: kzvd4729                                         created: 2020-03-02 00:43:59                      
...
struct polygon
{
  ...
  */
  ...
  int inConvex(point p)
  {
    ...
    int lo=1,hi=n-1;
    //for(auto x:h)cout<<x.x<<" ** "<<x.y<<endl;
    while(hi-lo>1)
    {
      int md=(lo+hi)/2;
      if(((h[md]-h[0])^(p-h[0]))>0)lo=md;
      else hi=md;
    }
    bool in=triangle(h[0],h[lo],h[hi]).inTriangle(p);
    if(!in)return -1;//return 1;
...
int main()
{
  ...
  polygon p;int n,q;cin>>n>>q;p.p.resize(n);
